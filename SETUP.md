...
**Type-aware rules are on** (`options.typeAware` in [`.oxlintrc.json`](.oxlintrc.json)), which adds the correctness rules that need type information — `no-floating-promises`, `unbound-method`, `no-base-to-string`, `no-misused-spread` and friends. They are powered by the **`oxlint-tsgolint`** dev dependency, whose version is pinned lockstep with `oxlint` (pnpm resolves it as oxlint's optional peer, so the lockfile shows `oxlint@<v>(oxlint-tsgolint@<v>)`). It ships prebuilt platform binaries with no install script, so `ignoreScripts: true` needs no `allowBuilds` entry and CI needs no Go toolchain for it — the existing `pnpm install --frozen-lockfile` in [`.github/actions/bootstrap`](.github/actions/bootstrap/action.yml) is enough. Cost is small: roughly 1.2s for a full pass against 0.7s without, so the `lint-staged` pre-commit hook runs it too. `oxlint`'s experimental `typeCheck` option stays **off** — `pnpm run typecheck` already owns compiler diagnostics.
