...
Outside this repo, `@ts-runtypes/bin`'s `getExePath()` resolves the per-platform `@ts-runtypes/binary-<os>-<arch>` package. **`MION_BIN=<path>` overrides that lookup for every lane** — the bundler plugins (behind their explicit `binary` option, which still wins) *and* the lint plugin, which resolves its binary through the launcher and takes no `binary` option:

```bash
MION_BIN=/abs/path/to/ts-runtypes pnpm run lint     # in the consumer project
```
