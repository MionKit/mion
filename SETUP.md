...
| drizzle     | `pnpm rtx core drizzle-translate` | Translate those suites onto the slim packages and typecheck the result. No container, no database. |
| drizzle     | `pnpm rtx release drizzle-e2e` | The full lane: translate, install from verdaccio, run against a real postgres, mysql and sqlite. `--dialect <d>` for one. |
