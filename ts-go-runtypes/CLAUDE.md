...
- [cachegen/](internal/cachegen/) — cache generation (runtype, typefunctions, purefunctions, operations, diskcache, builtinpurefns, hashid).
  ...
  - **The source files are found, not declared.** The loader reads the package's `src` and keeps whatever calls a registrar, so a file that registers cannot be missed: walking the import graph drops `circular-pure-fns.ts` (side-effect imported by nothing) and a hand-written list drops whatever someone forgets. It descends through `GetAccessibleEntries`, NOT the FS's `WalkDir`, which on an overlay filesystem delegates to real disk and cannot see a virtually served package. Spec and test files are skipped because the tarball excludes them. `cmd/gen-builtin-purefns` runs the same scan. A package registering through a wrapper of its own would be missed; that is the generic-lane problem, not this one.
