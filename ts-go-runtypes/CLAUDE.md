...
- [cachegen/](internal/cachegen/) — cache generation (runtype, typefunctions, purefunctions, purefnindex, purefnids, operations, diskcache, hashid).
  `purefnindex` serves the pure fns an INSTALLED package ships, the marker package's own built-ins included: one lane, no table compiled into the binary. Every mion build writes the package's own pure fns into ONE file in its output directory, `mion-pure-fns.json` (`constants.PureFnArtifactFileName`; shape in `purefnindex/artifact.go`), and a consumer's compiler reads only that file, never a bundle. When no artifact is found, the rows are extracted from the package's TypeScript sources with the same extractor a build runs. `@mionjs/run-types` lives on the source lane today: its dist is hollowed and its tarball ships `src`, which is why `src` must stay in that package's published `files`. Five rules hold it together:
  - **The bundle is never opened.** A bundle is whatever the library's bundler produced, minified, chunked, tree-shaken, in the library's emit mode; reading it costs its size and misses what the bundler dropped. The artifact is rendered by the library's own generate step from the whole program (`collectPureFnArtifact`, `resolver/render.go`), so a registration the bundle lost is still there, and the parse is bounded by the pure fns a package ships. `TestArtifact_BundleIsNeverOpened` pins it with a recording FS.
  - **An id is matched, never decoded.** It is the package plus a hash of the body that ships, so it says nothing about where that body lives; a served row keeps the library's id verbatim. A demanded id a package with rows does not produce is `PFE9012`; the marker package's sources failing to read or type check are `CFG004`, never a silent fallback; a package that ships nothing to serve at all is the runtime-only lane, reported per id as the warning `PFE9016`. An artifact this compiler cannot read (a newer `format`, or not an artifact) is the warning `PFE9017` naming the file, and two artifacts of one package giving one id different bodies is the error `PFE9018`; identical rows from an ESM and a CJS build merge silently.
  ...
  - **The marker package's source list is generated, not declared and not scanned for.** `cmd/gen-builtin-purefns` scans the package for a registrar call, narrows that to the files which produced an entry, and writes those paths as `purefnids.SourceFiles` beside the id constants, from one pass, so the constants and the served bodies cannot come from different files. Neither alternative works as well: following imports drops `circular-pure-fns.ts` (side-effect imported by nothing), a hand-written list drops whatever someone forgets, and scanning per session re-derives a constant answer, the package layout being fixed when the binary is built. A listed path the install lacks is `CFG004`; there is deliberately no fallback scan. Any OTHER package's sources are scanned for the registrar needle, since nothing about them is known when the binary is built.
  - **A dep resolves from the package that names it.** The walk of a package root for artifacts skips `node_modules` and hidden dirs (a consumer's `.mion` holds its own canonical copy under `types/` and served COPIES of other packages' rows); a row's dep on another package is located by a node_modules walk from that row's package root, so a nested install lands on the copy the package was built against. Only the building package's own rows go into its artifact: a workspace sibling the program reaches through the `source` condition belongs to its own.
  The output side: generate writes the canonical copy to `<genDir>/types/mion-pure-fns.json` (data, never a module, never GC'd) and returns the content on the Response; `mion compile` copies it into the tsconfig `outDir`, and each bundler adapter into its own output dir from its post-bundle hook, because generate runs at `buildStart`, before a bundler empties that dir.
