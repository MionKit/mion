...
- [cachegen/](internal/cachegen/) — cache generation (runtype, typefunctions, purefunctions, purefnindex, purefnids, operations, diskcache, hashid).
  `purefnindex` serves the pure fns an INSTALLED package ships, the marker package's own built-ins included: one lane, no table compiled into the binary. A package's built JS is read first (every mion build writes each pure fn as an entry tuple and its registration as `registerPureFn(<tuple>, '<id>')`, both recognised by literal shape); when no built file carries a tuple, the rows are extracted from the package's TypeScript sources with the same extractor a build runs. `@mionjs/run-types` lives on the source lane today: its dist is hollowed and its tarball ships `src`, which is why `src` must stay in that package's published `files`. Four rules hold it together:
  - **An id is matched, never decoded.** It is the package plus a hash of the body that ships, so it says nothing about where that body lives. A demanded id a package with rows does not produce is `PFE9012`; the marker package's sources failing to read or type check are `CFG004`, never a silent fallback; a package that ships nothing to serve at all is the runtime-only lane, reported per id as the warning `PFE9016`.
  ...
  - **A dep resolves from the package that names it.** The scan of a package root skips `node_modules` and hidden dirs (a consumer's `.mion` holds served COPIES of other packages' rows); a row's dep on another package is located by a node_modules walk from that row's package root, so a nested install lands on the copy the package was built against.
