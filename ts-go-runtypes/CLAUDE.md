...
- [cachegen/](internal/cachegen/) — cache generation (runtype, typefunctions, purefunctions, operations, diskcache, builtinpurefns, hashid).
  ...
  - **The source list is generated, not declared and not scanned for.** `cmd/gen-builtin-purefns` scans the package for a registrar call, narrows that to the files which produced an entry, and writes those paths as `purefnids.SourceFiles` beside the id constants, from one pass, so the constants and the served bodies cannot come from different files. The loader just resolves them against the root it found. Neither alternative works as well: following imports drops `circular-pure-fns.ts` (side-effect imported by nothing), a hand-written list drops whatever someone forgets, and scanning per session re-derives a constant answer, the package layout being fixed when the binary is built. A listed path the install lacks is `CFG004`; there is deliberately no fallback scan.
