...
func EnsureOutputHygiene(outDir, typesDir string) error {
	...
	// npm pack honours a nested .gitignore unless a .npmignore sits beside it; a gen dir placed inside a published
	// dir (a library built with `mion compile`) must ship these modules, since the emitted files import them.
