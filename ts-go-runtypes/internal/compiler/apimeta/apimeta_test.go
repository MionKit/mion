...
func TestWalkApi_ReadsResolvedOptionsAndDropsUndefinedKeys(t *testing.T) {
	...
	if _, present := remove["maxBodySize"]; present {
		t.Errorf("an undefined option must be absent, got %v", remove["maxBodySize"])
	}
...
`})
	...
	if strings.Join(tree.ById["r"].WidenedOptions, ",") != "sanitizeParams" {
		t.Errorf("widened %v", tree.ById["r"].WidenedOptions)
	}
	if _, present := tree.ById["r"].Options["sanitizeParams"]; present {
