...
type Opts<M> = {alwaysRun: false; validateParams: true; validateReturn: false; description: undefined; parser: {params: 'clone'; return: 'clone'}; isMutation: M; strictTypes: undefined; sanitizeParams: undefined};
...
export type Api = {
  ...
  users: {
    ...
    remove: {type: 1; handler: (id: number) => Promise<boolean>; options: {alwaysRun: false; validateParams: true; validateReturn: false; description: 'drop'; parser: {params: 'compact'; return: 'mutate'}; isMutation: true; strictTypes: true; sanitizeParams: undefined}; types?: {params: [id: number]; return: boolean; headers: never; isAsync: false}};
  };
...
func TestWalkApi_ReadsResolvedOptionsAndDropsUndefinedKeys(t *testing.T) {
	...
	parser, _ := remove["parser"].(map[string]any)
	if parser["params"] != "compact" || parser["return"] != "mutate" {
		t.Errorf("remove parser %v", parser)
	}
...
type Api = {r: {type: 1; handler: (n: number) => Promise<number>; options: {alwaysRun: false; validateParams: true; validateReturn: false; description: undefined; parser: {params: 'clone'; return: 'clone'}; isMutation: undefined; strictTypes: boolean; sanitizeParams: undefined}; types?: {params: [n: number]; return: number; headers: never; isAsync: false}}};
