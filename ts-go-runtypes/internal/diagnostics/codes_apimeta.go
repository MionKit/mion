...
//
// Levels follow the catalog rule. MET001 / MET002 / MET005 drop the site, so
// nothing is injected and the call throws `route-metadata-not-found` at
// runtime under `bundled`: LevelError. MET003 is the same runtime failure for
// a widened id under `bundled`, where the site cannot be resolved at all:
// LevelRuntimeError (the build ships, the call fails). MET004 is its `mixed`
// twin, where the fetched lane still answers: LevelWarning. MET006 ships a
// method whose bundled options leave one field unset: LevelWarning.
const (
...
func init() {
	for _, definition := range []Definition{
		...
		{Code: CodeApiMetaRouteWidened, Family: FamilyMarker, Level: LevelRuntimeError, Scope: ScopeNotSource, Title: "The route id at a dispatch site was widened to `string`, so nothing can be bundled for it"},
