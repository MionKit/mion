...
// collapsedIntersectionID mirrors the serialize-side collapse so two structurally-equivalent post-collapse
// types share one structural id (and one wire hash). The classification must stay in sync with
// internal/cachegen/runtype/intersection_collapse.go. Rules:
//   - `A & B` resolving to an object literal (the checker already merged the property set) → the object
//     literal's id;
//   - `string & {__brand}` → "<primitive-id>&{<sorted decorator ids>}", so brand ORDER doesn't matter
//     (A & B == B & A) but brand CONTENT does;
//   - `string & number` (any incompatible primitive pair) → "never"; everything-any → "unknown".
func (computer *Computer) collapsedIntersectionID(tsType *checker.Type) string {
	...
	for _, member := range members {
		...
		case memberFlags&checker.TypeFlagsObject != 0,
			// The bare `object` keyword (TypeFlagsNonPrimitive) — mirror the serialize side so `object & {sentinel}`
			// hashes the base instead of silently dropping the base member.
	...
	if primary != nil && len(objectMembers) > 0 {
		...
		for _, objectMember := range objectMembers {
			// Format brands are lifted out of TypeMeta on the serialize side; mirroring the lift in the ID is what
			// keeps two intersections that differ only in their format brand hashing distinctly. Canonical params
			// (sorted keys, recursed) make key order in `{maxLength: 10}` irrelevant to the cache key. Multiple
			// same-family brands MERGE (sibling conjunction: `$ref`-target ∧ local keyword), so `A & B` and `B & A`
			// fold one canonical key.
			if annotation := FormatAnnotationFromType(computer.typeChecker, objectMember); annotation != nil {
				annotations = append(annotations, annotation)
				continue
			}
			// A pure `{__rtFormatBrand}` member is the TS-only nominal brand and must NOT enter the structural id,
			// else a branded format would stop deduping with its unbranded twin.
		...
		if merged, ok := MergeFormatAnnotations(annotations); ok {
			...
			// Unmergeable stack — the serialize side fails the build loudly; fold a deterministic sorted concat so
			// this id stays stable in the meantime.
	...
	// Builtin-class × brand (`FormatDate<P>` → `Date & {brand}`): mirror the serialize-side
	// splitBuiltinClassBrand so the id reflects the node's REAL shape — a Date class node plus a format key,
	// not an object literal whose members include the sentinels. Otherwise the id encodes the brand props as
	// properties, inconsistent with the projected KindClass/SubKindDate node and divergent from how an atomic
	// format's id keeps the brand out of the member set.
	...
	if len(objectMembers) > 0 {
		// Structural format brands and slot sentinels among object-only intersections
		// (`unknown[] & {__rtFormatName?: …}`): lift them before the merged hash. memberIDs skips the sentinel
		// props, so the remaining hash equals the sentinel-free object's and the brand contributes only the
		// format key, mirroring the serialize side.
		...
		for _, objectMember := range objectMembers {
			// Labeled-tuple sentinel (`[A, B] & {__rtLabels?: ['x', 'y']}`): lift the labels and fold them INTO the
			// tuple id below — the per-element fold the type-first labeled tuple gets — so the value-first object
			// form and `[x: A, y: B]` share one id. Twin of the serialize side's label-aware projectTuple.
			...
			if specs, ok := PatternPropsFromMember(computer.typeChecker, objectMember); ok {
				// Source + value pin the semantics; the key brand exists only for mock pools and stays out of the id
				// (its pattern equals the source by construction).
		...
		if merged, ok := MergeFormatAnnotations(annotations); ok {
			...
			// Unmergeable stack — the serialize side fails the build loudly; fold a deterministic sorted concat so
			// this id stays stable in the meantime (same fallback as the primitive branch above).
			keys := make([]string, 0, len(annotations))
			for _, annotation := range annotations {
				keys = append(keys, FormatAnnotationStructuralKey(annotation))
			}
		...
		if restCount == 1 && (formatKey != "" || containsKey != "" || haveTupleLabels) {
			// Single base ∧ sentinel(s): hash the base AS ITSELF plus the contains / pattern folds and format key —
			// the serialize side projects the base node directly (array / record / class), never a merged
			// objectLiteral. Lifted tuple labels fold INTO the tuple id (per-element, the type-first spelling's
			// fold), and only for a tuple base whose element count they cover; anything else is a hand-rolled
			// sentinel, ignored on both sides.
		...
		// Tuple ∩ tuple — merge slot-wise (tuplemerge.go) so the id equals the equivalent hand-written tuple's;
		// a genuine conflict hashes as never (over-rejects, never silently under-validates). Twin of the
		...
		if restCount >= 2 && AllTupleOrArrayTypes(computer.typeChecker, restMembers) {
			...
			for _, pick := range picks {
				// Same per-slot formulas as the plain tuple id (typeid.go) — optional slots through optionalChildID +
				// "?", rest through "..." — so the merged id is byte-equal to the hand-written tuple's.
				var child string
				switch {
				case pick.Fold != nil:
					// A folded slot is already undefined-stripped, so it needs only the `?` marker.
		...
		// TupleType, and objectID's array/promise/class branches call GetTypeArguments unconditionally, which
		// crashes on intersection types in tsgo.
		ids := computer.memberIDs(tsType, false)
		// Embed call signatures alongside the members, exactly as objectID does for a written object literal, so
		// a CALLABLE intersection (`func & {props}`, the value-first authoring of a callable interface) converges
		// with the type-first `{(): r; props}`. Without this the call signature is dropped from the id and the
		// two forms diverge even though their projected nodes match.
...
// builtinClassNamesID is the id-side mirror of the serialize-side builtinClassNames
// (internal/cachegen/runtype/intersection_collapse.go). The two MUST list the same names or the
// structural id and the projected node disagree on which members are builtin-class bases.
var builtinClassNamesID = map[string]bool{"Date": true, "Map": true, "Set": true, "RegExp": true}

// splitBuiltinClassBrandID detects the `Builtin & {brand}` shape among an intersection's object members:
// exactly one recognised builtin-class member plus at most one TypeFormat-brand member and any number of
// `__rtContains` sentinel members (a FormattedSet's `contains` slot). Returns the class member and the key
// folded into the id — contains key then canonical format key, the order the object branch uses, so two
// brands differing only in params hash distinctly. Mirrors the serialize-side splitBuiltinClassBrand.
