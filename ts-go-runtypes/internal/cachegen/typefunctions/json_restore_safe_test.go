...
func TestRestoreFromJsonSafe_IndexSignatureRebuildTriggers(t *testing.T) {
...
func TestRestoreFromJsonSafe_PatternKeysAreCopiedNotDropped(t *testing.T) {
	num := &reflection.RunType{ID: "num", Kind: reflection.KindNumber}
	method := &reflection.RunType{ID: "pm", Kind: reflection.KindMethodSignature, Name: "m"}
...
func TestRestoreFromJsonSafe_RebuildPositions(t *testing.T) {
	str := &reflection.RunType{ID: "str", Kind: reflection.KindString}
	propA := &reflection.RunType{ID: "pa", Kind: reflection.KindProperty, Name: "a", IsSafeName: true, Child: makeRef("str")}
	...
	if !strings.Contains(circular, "r0.list = v.list;v = r0;") || !strings.Contains(circular, "v.list[1] = IRLg_self(v.list[1])") {
