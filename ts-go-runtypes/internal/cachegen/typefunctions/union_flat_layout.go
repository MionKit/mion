...
//     constructors), independent of the member order (which is the checker's,
//     not the source's), and skipped when the class is unregistered
//     (`cs_<name>` undefined). There is deliberately NO `instanceof` arm: a
//     subclass instance is not the declared class, so it takes the structural
//     road below like any other value of the declared shape, and its undeclared
//     keys are the unknown-keys machinery's business (encoder strategy, strip),
//     never a side channel of the class lane.
...
//     class instance, a subclass instance, or a plain object assignable to a
//     class-union position (best-effort: two same-shape classes fall to the
//     first, which is harmless since a structural decode rebuilds the declared
//     class either way).
//
// A class member therefore appears in TWO arms (exact + structural) selecting
