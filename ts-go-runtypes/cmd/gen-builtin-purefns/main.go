...
// registration sources in packages/ts-runtypes/src. One row per built-in fn:
...
const (
	markerPkgRel = "../packages/ts-runtypes"
...
func render(entries []purefunctions.Entry) ([]byte, error) {
	...
	b.WriteString("// built-in pure-fn sources in packages/ts-runtypes/src.\n\n")
