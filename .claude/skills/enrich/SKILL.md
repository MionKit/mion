...
| --- | --- |
| [packages/ts-runtypes/skills/rt-enrich-types/SKILL.md](../../../packages/ts-runtypes/skills/rt-enrich-types/SKILL.md) | the WORKFLOW: running `gen` / `gen --update` / `gen --prune` / `gen --translate` / `check`, the mirror directory layout, the `@rtType`/`@rtIds`/`@rtOrphan`/`@todo` tag contract, translations + the tsconfig `i18n` block |
