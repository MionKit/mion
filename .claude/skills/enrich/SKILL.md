...
| --- | --- |
| [packages/ts-runtypes/skills/rt-enrich-types/SKILL.md](../../../packages/ts-runtypes/skills/rt-enrich-types/SKILL.md) | the WORKFLOW: running `enrich` / `enrich --update` / `enrich --prune` / `enrich --i18n` / `enrich --no-emit` / `enrich --require-complete`, the mirror directory layout, the `@rtType`/`@rtIds`/`@rtOrphan`/`@todo` tag contract, translations + the tsconfig `i18n` block |
