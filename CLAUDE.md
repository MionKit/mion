...
- [drizzle-orm-pg-core](packages/drizzle-orm-pg-core/) / [-mysql-core](packages/drizzle-orm-mysql-core/) / [-sqlite-core](packages/drizzle-orm-sqlite-core/) (`@mionjs/drizzle-orm-<dialect>-core`) — drizzle-orm extensions, one package per dialect; versioned `<drizzle major.minor>.<patch>` via the `versionLine: "drizzle-orm"` package.json marker (NOT on the lockstep train); the hand-owned generator config lives in [drizzle-dialects.json](drizzle-dialects.json) at the repo root.
...
- All JS: `pnpm test` (all 17 vitest projects). Single file: `pnpm exec vitest run <pattern>`. Single package: `pnpm --filter <name> test`.
