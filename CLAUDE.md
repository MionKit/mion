...
- **⚠️ Found a bug outside your current task's scope? Surface it, then TRIAGE it — filing is the fallback, not the default.** Any defect discovered along the way (a fuzzer finding, a soundness-tripwire message in test output, a latent bug a new test exposes, a doc-vs-code contradiction) is first **surfaced to the user in your reply**: what it is, where it came from, and whether it predates your change (bisect if cheap). Then judge the fix, and say which way you went:
  - **Small and self-contained → FIX IT in the same PR**, even when unrelated to the task. A one-line deletion, a wrong constant, a stale comment, a missing guard, a dead class: filing a spec for these costs more than the fix, and a `docs/todos/` full of trivia buries the entries that matter. Ship it with its own commit and its own test where the change is testable.
  - **Large, risky, or a real design question → file it** as a spec under [docs/todos/](docs/todos/) with the evidence and a concrete fix plan, and leave it. "Large" means: it needs a decision you cannot make alone, it touches a load-bearing invariant, it would balloon the diff past what a reviewer can hold, or you cannot verify the fix inside this task.
  - **Unsure → ask.** One sentence stating the fix you have in mind and its size beats both guessing and reflexively filing.

  Two things stay absolute: never let a finding live only in chat, and never let a `docs/todos/` spec be a way to avoid five minutes of work. A filed spec must be justified by size or risk, not by tidiness.
