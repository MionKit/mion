...
- **The mion docs website is folded in:** it is the `mion` site of the one Nuxt install at [container/website/](container/website/) (content under `sites/mion/`), served/built with `pnpm rtx website … --site mion` and deployed to mion.pages.dev by `website-deploy.yml`.
- **One pre-publish gate:** `pnpm rtx release e2e` covers both families. `pack.mjs` packs every non-private `packages/*` (21 tarballs), verdaccio serves both scopes locally, and the mion side rides two consumer lanes under [container/pre-publish-e2e/](container/pre-publish-e2e/) — `mion-consumer/` (vite + vitest: JSON/binary round-trips, packaged-tarball inspection, the `@mionjs/devtools/eslint` transport, and the production build's inlined types + mappers) and `mion-bun/` (a real `Bun.serve` mion server). ⚠️ `tarballs/` is also what the release publishes from, and `@mionjs/*` is NOT on the release train yet — `publish-tarballs.mjs` filters to `ts-runtypes-*` until the merge plan's step 6 unifies the versions.
