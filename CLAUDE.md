...
Cross-package deps use the `workspace:*` protocol. All devDependencies live root-level, never per-package, with TWO exceptions (dev deps never reach a consumer): each drizzle dialect package carries `@mionjs/run-types: workspace:*` to satisfy its own peer in the workspace, and `@mionjs/run-types` carries `@mionjs/devtools` plus `@mionjs/bin-compiler`, the `mion` CLI its build runs. 
⚠️ That link is one way: `@mionjs/devtools` NEVER depends on or imports `@mionjs/run-types`, or pnpm sees a workspace cycle. Its tests copy the built run-types into their fixtures by path instead; `pnpm run check:tree` fails on any workspace dependency cycle. Why and how: [packages/devtools/CLAUDE.md](packages/devtools/CLAUDE.md).
...
- [router](packages/rpc-router/) — HTTP routing and request handling. [client](packages/rpc-client/) — client-side utilities.
...
- All JS: `pnpm test` (all 24 vitest projects). Single file: `pnpm exec vitest run <pattern>`. Single package: `pnpm --filter <name> test`.
- Only what a branch touched: `pnpm miondevx core test-pr [--base <ref>] [--list]` ([scripts/core/test-pr.mjs](scripts/core/test-pr.mjs)), the changed packages plus their dependents, from the committed diff. CI's JS suite runs it on pull requests and saves a narrower `js-pr` green marker that a push to `main` never accepts.
- If one full run OOMs, `pnpm run test:ci` runs the SAME 24 projects in 7 batches, one vitest process per batch (resolver processes are ~200 MB each). The batches live in [scripts/core/test-batches.mjs](scripts/core/test-batches.mjs) and only GROUP the names `vitest.config.ts` declares: `pnpm run check:test-batches` (a CI gate, and the run's own preflight) fails if a project sits in no batch or in two. Adding a project means adding it to a batch.
...
- `pnpm run typecheck` type-checks EVERY package, and **ONE config per package serves both the check and the editor**: `tsconfig.json` is non-composite, `noEmit`, and declares NO `references`, so a cross-package import resolves through the `source` export condition instead of a sibling's built declarations. A fresh checkout has no dists, which is the TS6305 "has not been built from source file" wall, and it is also why the editor was always fine where `tsc -p` was not. `tsconfig.build.json` is the emit half and the ONLY place dist appears: it re-enables `composite` / `incremental` / `outDir`, and its `references` name sibling BUILD configs because a reference target must be composite (the root tsconfig references the build configs for the same reason). NO package keeps a second config for checking: `run-types` and `devtools` were migrated too (`run-types` builds with `mion compile --tsconfig tsconfig.build.json`, `devtools` with `tsc --build tsconfig.build.json`), and `examples` emits nothing so it has no `tsconfig.build.json` at all. `pnpm -r` skips a package that has no `typecheck:test` script and says nothing, so `pnpm run check:typecheck-coverage` ([scripts/core/typecheck-coverage.mjs](scripts/core/typecheck-coverage.mjs), the root typecheck's own preflight and a CI gate) fails if a package has none, or if a file it ships sits in no project. Adding a package means adding that script.
...
- **Two markers + demand-driven caches** — `InjectRunTypeId<T>` (injects typeId) drives the reflection cache; `InjectTypeFnArgs<T, Fn>` (injects typeId + opaque 4-char fnHash) drives per-family caches.
...
- **The mion request pipeline sits on top** — `@mionjs/router` executes the typed handlers declared through the helpers `createMionRouter(opts)` returns (`mion.route()` / `mion.middleware()`, the ONE way to initialize the router; the options ride by type into every helper) using the compiled validators/serializers the runtypes caches provide (via `@mionjs/core`'s reflection adapter); the `platform-*` adapters wrap the router per runtime.
...
Run `pnpm miondevx <area> <command>`, or `pnpm miondevx --help` to list them. Areas: core, website, bench, container, env, release, card.

`tools/` holds private dev tools with their own dependencies, outside `packages/` so no CI lane, root test run or root lint touches them. [tools/code-card/](tools/code-card/) renders a markdown card to a shareable PNG (`pnpm miondevx card`, and `pnpm miondevx card test` for its own checks); agents use the [code-card skill](.claude/skills/code-card/).
