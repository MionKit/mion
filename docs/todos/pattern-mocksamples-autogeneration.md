...
**Generation runs in the #1 sidecar with a bundled randexp.** JS regexes deserve a JS-semantics generator; the sidecar already executes under node/bun. Add op `generate` to the sidecar protocol: `{id, op:'generate', source, flags, count, seed, maxLength?}` → `{id, values, error}`. `randexp` (its `ret`/`drange` come along transitively) is an exact-pinned `dependency` of the private sidecar workspace package (mind `minimumReleaseAge` when adding), bundled into the committed sidecar bundle by its vite build — **never a runtime dependency of user projects**.
