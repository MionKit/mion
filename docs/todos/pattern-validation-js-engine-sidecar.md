...
- **Authoring home: a new private workspace package** (e.g. `packages/ts-runtypes-sidecar/`, `"private": true` — the `packages/examples` precedent: a real workspace member that never publishes; keep it out of `PUBLISHED_PACKAGE_DIRS`, `repo-contracts.test.ts:42-45`). TS source with its own vitest unit tests, and it falls inside `pnpm run format`'s `packages/**/*.ts` scope and the lint sweep for free (a loose `.mjs` under `ts-go-runtypes/` would sit outside all three). `randexp` (needed by the autogeneration follow-up) is declared here as an exact-pinned per-package `dependency`; root-level stays devDeps-only per workspace policy, and `minimumReleaseAge` applies on add.
...
1. **Sidecar package + codegen.** New private workspace package implementing the protocol (stdin JSON `{v, jobs:[{id, op:'validate', source, flags, samples}]}` → stdout `{v, results:[{id, ok, offenders, error}]}`; `v` + op strings let the autogeneration todo add `generate` without reshaping), with its own vitest unit tests; `build` (vite lib mode) → single-file unminified bundle; `CODEGEN.sidecar` lane copies it to the committed `internal/jsengine/sidecar.bundle.mjs` for `//go:embed`.
...
3. **Wire into validation — JS engine only.** In `pattern.go`, queue every pattern+samples job during render and drain once per pass through `Options.JSEngine` (one batch), emitting FMT001 from the JS verdicts; the per-session memo on `(source, flags, samples)` keeps steady-state watch rebuilds spawn-free. Delete `re2Pattern`, the user-input `regexp.Compile`, and `reportUncheckedPattern`; FMT004 becomes the missing-runtime Error above. FMT003 bounds checking is pure Go (UTF-16 counting, no regex) and unchanged. `compile` gains full validation automatically (same render path); `enrich` stays off the validation path (no format emitters).
...
- **Go:** rework `format_sample_validation_test.go` with a fake engine — JS-only pattern (lookbehind) + bad sample ⇒ FMT001; all-valid ⇒ no diagnostic; a divergence case (a sample RE2 would have accepted but JS rejects, e.g. `\u2028` against `.`) ⇒ FMT001, pinning that the JS engine is the sole authority; engine unavailable + patterns present ⇒ FMT004 on every pattern-bearing site, never a silent skip (the `_UncheckedPatternFlagSuppresses` lane dies with the flag); engine unavailable + zero patterns ⇒ builds clean. Keep the suite's paired static/reflect `getRunTypeId` fixtures and hash-equivalence assert (marker coverage rule; exemplar `resolver/atomic_test.go:953`).
...
With RE2 gone there is no second implementation to keep in parity, so no fallback-fidelity suite. What remains worth fuzzing is robustness: a `patternSidecar` suite (entry in the `FUZZ` table, `scripts/rt.mjs:56-67`) throws random garbage at the real sidecar under node — arbitrary pattern sources and flags (including invalid regex syntax), pathological samples, oversized batches — and asserts the resolver always receives a well-formed verdict or a clean engine error: never a hang, a crash, or a partial batch. The generate-side round-trip oracle lives in the autogeneration todo.
...
- [ ] RE2 is gone from sample validation (`re2Pattern` + the user-input `regexp.Compile` deleted); FMT004 is the missing-runtime Error, fired only when patterns exist and no engine runs; `allowUncheckedPatterns` is fully removed (tsconfig key, CLI flag, plugin option, docs row).
- [ ] Sidecar lives as a private workspace package (tests, lint, format all apply); its vite-built bundle is committed at `internal/jsengine/sidecar.bundle.mjs` behind the drift-gated `codegen sidecar` lane and loaded via `//go:embed`; binary packages' contents unchanged.
...
Estimated size: ~950 changed lines (≈400 new engine/sidecar, ≈400 deletions including the RE2 bridge and the `allowUncheckedPatterns` surface, ≈200 tests/docs); 2-3 focused sessions. Land before the autogeneration todo.
