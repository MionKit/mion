...
**Validation authority: the sidecar validates EVERY pattern's samples when a runtime exists — not only the RE2-incompatible ones.** The samples' one job is to satisfy the JS runtime validator (`emitPatternTest`'s hoisted `new RegExp`, `registerFormatPattern`'s load-time check), and RE2 only approximates it even on the shared subset: `.` excludes `\u2028`/`\u2029` in JS but not RE2; case folding under `i` diverges without `u` (Kelvin sign `K`); no-`u` JS matches UTF-16 code units where RE2 matches runes (`^.$` vs `'😀'`); JS `\s` includes `\uFEFF` while RE2's does not — today each of these is a silent false accept or a false FMT001. Verdicts are memoized per session on `(source, flags, samples)` (validation is pure), so watch-mode rebuilds spawn nothing once warm.

**Failure semantics:** spawn failure, timeout, or malformed output ⇒ treat as "no JS runtime available" — RE2-compilable patterns fall back to today's in-process RE2 check (best effort), RE2-incompatible ones get FMT004 with its message updated to say "install node/bun or pass --js-runtime"; `allowUncheckedPatterns` keeps suppressing FMT004, whose meaning narrows to this one case. Diagnostics severity semantics are unchanged (`internal/diagnostics/catalog.go:19-32`; Error halts via `surfaceDiagnostics` + `failOnError`, `unplugin.ts:427,455,647-648`).
...
3. **Wire into validation — JS engine authoritative.** In `pattern.go`, queue every pattern+samples job during render and drain once per pass through `Options.JSEngine` (one batch), emitting FMT001 from the JS verdicts; the per-session memo on `(source, flags, samples)` keeps steady-state watch rebuilds spawn-free. RE2 (`regexp.Compile`, `pattern.go:123`) is demoted to the no-runtime fallback for compilable patterns; `reportUncheckedPattern` disappears. FMT003 bounds checking is pure Go (UTF-16 counting, no regex) and unchanged. Engine unavailable ⇒ fallback semantics above. `compile` gains full validation automatically (same render path); `enrich` stays off the validation path (no format emitters).
...
- **Go:** rework `format_sample_validation_test.go` with a fake engine — JS-only pattern (lookbehind) + bad sample ⇒ FMT001; all-valid ⇒ no diagnostic; a divergence case (a sample RE2 accepts but JS rejects, e.g. `\u2028` against `.`) ⇒ the JS verdict wins; engine unavailable ⇒ the RE2 fallback still validates compilable patterns and FMT004 fires only for incompatible ones; `allowUncheckedPatterns` + unavailable ⇒ silent skip. Keep the suite's paired static/reflect `getRunTypeId` fixtures and hash-equivalence assert (marker coverage rule; exemplar `resolver/atomic_test.go:953`).
...
New suite `patternParity` (entry in the `FUZZ` table, `scripts/rt.mjs:56-67`): random patterns + samples from a constrained generator that excludes the documented divergence classes ⇒ the RE2 fallback's verdict must agree with the sidecar's JS verdict. This compare-two-implementations oracle pins the `re2Pattern` flag-translation bridge (`pattern.go:365-381`) so the no-runtime fallback stays faithful; the known divergence classes themselves get explicit unit tests asserting the JS verdict is the one users see whenever a runtime exists.
