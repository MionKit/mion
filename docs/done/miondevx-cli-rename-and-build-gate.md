...
- **The e2e lanes build only when they will pack.** `release e2e` and `release drizzle-e2e` run in CI on a checkout with NO Go submodule: the build job packs the tarballs once, the lanes download them as an artifact and only need `pnpm install` plus the prebuilt image. Gating them unconditionally broke all five drizzle lanes on the first CI run (`go: cannot load module third_party/tsgolint`). Their rows now decide per call: build when `tarballs/` holds no `.tgz` or `--pack` is given, never on `--backend npm`. On a dev host with no tarballs the lane packs itself, which builds the `@mionjs/*` dists through the devtools plugin, so the engine is still guaranteed there.
