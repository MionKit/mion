...
- **Server half, automatic.** The `mion-batches` plugin is always registered. At `transform` of the module that imports `@mionjs/router` and names `initMionRouter` it looks for the file, verifies the checksum and appends the import (`./`-prefixed, `map: null`). No file: nothing injected, the server has no batches. A file that exists but was injected nowhere fails `vite build`. `injectInto` is gone; the message says to import `@mionjs/router` directly in the module that calls `initMionRouter`.
