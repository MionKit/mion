...
- Each package owns its manifest: `packages/drizzle-orm-pg-core/manifests/pg.manifest.json` etc. The hand-owned config moves to `packages/drizzle-orm-manifests/dialects.json` (no package.json, invisible to the workspace); rows gain per-dialect `packageDir`, `proxy`/`manifest` become packageDir-relative.
