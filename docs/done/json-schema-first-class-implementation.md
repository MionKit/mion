...
- **`pattern` keyword: ENABLED** per the 04-migration-plan §1 resolution — sample-less validation works end-to-end (the brand rides `{source, flags}` params); mocking initially threw the targeted register-samples error, then main's sidecar-generated sample pools landed mid-PR and the define suite now pins that a schema pattern MOCKS from the build-generated pool. The `FromJsonSchema` internals spell the brand as raw `TypeFormat<string, 'stringFormat', P>` because the `TF.String` alias's `pattern` param REQUIRES `mockSamples` (structurally identical, ids converge).
...
### Renamed after the fact: `jsonSchema` → `runTypeFromJsonSchema` (2026-07-31)

The builder this todo introduced was renamed on `feature/json-schema-rollout`, before the surface ever reached npm. **Every reference below that spells it `jsonSchema` predates the rename**; the entry point is `runTypeFromJsonSchema`, from the same `@ts-runtypes/core/json-schema` subpath.

- **Why.** Unlike every other builder it is imported BARE — there is no `RT.*` namespace to carry the "this builds a RunType" signal, so `RT.object` can be named for what it describes and this one cannot. Three concrete costs of the short name: `const jsonSchema = {…}` is precisely what a caller names their schema literal (a shadowing collision, loud rather than silent, but real); the name stated the input and not the output, which the `RunType<FromJsonSchema<S>>` return makes load-bearing; and it was direction-ambiguous against the planned OUTPUT-direction `createJsonSchemaFn`. `fromJsonSchema` was the runner-up (it pairs with the existing `FromJsonSchema<S>` type) and was rejected for naming only the input half.
- **Cost was zero, which is why it happened now.** The `./json-schema` subpath had never been published — npm's then-latest `@ts-runtypes/core@0.11.0` exported only `.`, `./schema`, `./formats`, `./formats/temporal`. No deprecation, no alias, no migration path.
- **It was a pure identifier rename, and the marker design is why.** Site detection is structural (a trailing `InjectRunTypeId<T>` param), so the devtools plugin holds ZERO references to the callee name and the Go tree holds one, in a comment. Nothing in the resolver, the scanner, or the wire protocol moved. `src/json-schema/jsonSchema.ts` was `git mv`'d to `runTypeFromJsonSchema.ts` to match its single export.
- **What deliberately kept the old spelling**, because none of it names the function: the internal carrier discriminator `{type: 'jsonSchema', …}` (written by `builderResult`, read by nothing); the benchmark form labels `ts-go(jsonSchema)` / `ts-runtypes (jsonSchema)` and the `jsonSchema` / `jsonSchema binary` serialization column keys (they name the AUTHORING FORM, are keys into generated bench data, and are quoted in website prose); the `distinctness.test.ts` case labels; and `jsonSchemaOutput.proto.test.ts`, which is the output direction.
- **Gate:** `rt core build` clean, root `typecheck` exit 0, full `pnpm test` 250 files / 9857 passed / 0 failures, `oxlint` clean.
- **One edit no host gate covers.** `typecost/typecost.mjs` embeds the baseline probe as a literal TS source string (`"runTypeFromJsonSchema({type: 'string'})"`); it and the `competitors/ts-runtypes/*.ts` maps only compile inside the benchmark container, per [benchmark-competitor-maps-never-typechecked.md](../todos/benchmark-competitor-maps-never-typechecked.md). Corrected by inspection, unverified until that lane runs.
