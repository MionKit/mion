...
- **The app names the key.** `mionSyncRoutes` is a plain middleware, not a keyed entry to spread: `{syncRoutes: mionSyncRoutes, ...routes}` and `useSyncRoutes(middlewares.syncRoutes)`. Nothing keys on a fixed id any more; `MION_ROUTES.syncRoutes` is gone. Placed first, it runs before every other middleware and route of the chain.
