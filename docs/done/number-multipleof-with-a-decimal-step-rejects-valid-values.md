...
- **Integer formats** (`integer: true`: `Integer`, `Int8` to `UInt32`, `PositiveInt`, ...) only take a whole `multipleOf`. A fractional one is a build error from `ValidateParams`. They keep the exact `(v % n === 0)`.
- **Other number formats**: a whole step keeps `%`. A fractional step emits a relative-tolerance check, shared by the validator and the error list:
  `(Math.abs(v / 0.01 - Math.round(v / 0.01)) <= Math.abs(v / 0.01) * 8.881784197001252e-16)`.
- **New param `multipleOfTolerance`**, per type: `TF.Number<{multipleOf: 0.01; multipleOfTolerance: 1e-9}>`. Default `4 * Number.EPSILON`: the quotient carries at most ~1.5 epsilon of rounding noise, so it accepts every decimal multiple and nothing else (checked over 2.8M true multiples on 7 steps and 3.6M non-multiples of 0.01). Build errors: tolerance without a fractional `multipleOf`, or tolerance outside (0, 1). It never fails on its own, so it is a non-failing param for FriendlyText (Go `nonFailingParams`, TS `NonFailingParams`) and is listed in `schemadoc.GenericParamKeys`. JSON Schema output drops it (not a standard keyword).
- **JS twin** `packages/run-types/src/mocking/isMultipleOf.ts`, used by the number mock (`snapToMultiple`) and by `childMatch.ts`, whose `value % param` had the same bug for a fractional step.
- Other readers needed nothing: binary sizing reads only min / max / integer, and the typeid merge already refuses to fold a fractional `multipleOf`.
- The `number({params})` / `currency({params})` builder-cost budgets rose by 2 instantiations for the extra `NumberParams` key.

Tests: Go `TestNumberValidate_FractionalMultipleOf` and new `TestValidateParams` rows; JS suite cases `number_multipleOf_decimal` and `number_multipleOf_tolerance` (both marker shapes, error list, deserialize, mocks); `test/suites/mocking/isMultipleOf.test.ts` (twin, child matcher, every cent in ±1000.00 through the generated validator).

Docs: the page this spec named only exists on the docs-restructure branch, so the change went into `02.runtypes/02.guide/02.type-formats.md` section Custom Formats (table row, one paragraph, a `Price` in `type-formats-custom.ts`). That branch carries it over when it rebases.
