...
## What shipped
- The folders are `packages/rpc-router` and `packages/rpc-client`. npm names and vitest project names are unchanged, so `scripts/core/test-batches.mjs` and `scripts/ci/lanes.mjs` needed no edit. SETUP.md, the workflows and the container e2e setups never named the old folders.
...
- `scripts/release/e2e.mjs` used to guess `packages/router` from `@mionjs/router`. It now reads each consumer package's version by npm name through the new `versionsOf()` in `scripts/lib/publish-order.mjs`, tested in `packages/devtools/test/publish-order.test.ts`.
- The root `tsconfig.json` alias `@mionjs/*` → `./packages/*` assumed folder = npm name, and Bun follows it with no fallback, so the platform-bun suite could not load `@mionjs/router`. The alias now lists every package whose folder differs (the two rpc folders and the five `private-` ones, which the earlier rename left broken the same way). Contract block "the root tsconfig maps every @mionjs name to its own folder" in `packages/devtools/test/repo-contracts.test.ts` checks both ways: every package resolves to its folder, and every explicit alias names a package that exists.
