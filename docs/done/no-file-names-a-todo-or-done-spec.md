...
## Plan, as built (2026-09-02)

The sweep on the day of the fix returned 67 hits in 56 files, not the ~20 the snapshot above lists: the
Go tree (`ts-go-runtypes/cmd` and `internal`), the run-types fuzz harness, the test-server README and
several run-types and platform tests carried references too.

- Every hit was rewritten in place. Where the surrounding comment already carried the reasoning (the
  large majority), the spec name was simply dropped. Where the comment leaned on the link for content,
  the spec was read (when it still existed) and one or two sentences now say what it established: the
  palette recipe in the website CLAUDE.md, the closed data set and shape-based binary recognition in
  the two `docs/maybe/` specs, the swallowed container exit in WEBSITE-DOCGEN.md, the module-graph
  duplication behind the three `@mionjs/core` singletons, the runtypes `onSiteFilesChanged` callback in
  the SFC test, and a concrete replay hint in the soak tripwire message.
- Two skills (`create-todo`, `implement-todo`) named `seeded-mock-data.md` and
  `union-validate-dedup-object-guard.md` as example specs by bare filename. Both specs were already gone,
  so the examples were rewritten to describe the shape instead.
- The contract lives in `packages/devtools/test/repo-contracts.test.ts`: a `git grep` over tracked files
  for `docs/(todos|done)/<name>.md`, filtered by one exemption regex (the two spec directories,
  `CHANGELOG.md`, `ts-go-runtypes/third_party/`, every `_deps/` and `node_modules/`). The matcher is a
  pure function over (path, text), so three sibling tests prove it flags a reference in code, a workflow,
  a doc, a skill or a `docs/maybe/` spec, ignores the exempt paths, and does not trip on a bare mention
  of the directories.
- Bare `<name>.md` mentions are NOT caught by the contract. Too many legitimate `.md` files share the
  naming style, so the check would either miss them or false-positive; the path form is the one that
  can be enforced mechanically, and the hand sweep above cleared the bare mentions that clearly pointed
  at a spec.
- No website docs: contributor-only.
