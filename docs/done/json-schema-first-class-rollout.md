...
- [benchmark-competitor-maps-never-typechecked.md](../todos/benchmark-competitor-maps-never-typechecked.md) — this spec's ground rule 3 ("compilation is the proof") is not true today: nothing in CI runs `tsc` over `container/benchmarks`, which is how the `schemaCases.ts` drift above survived. Needs an in-container check, since the deps never exist on the host.
- [typia-validationerrors-column-calls-a-removed-api.md](../todos/typia-validationerrors-column-calls-a-removed-api.md) — every one of typia's 195 `buildErrors` thunks calls `typia.createValidateFn`, which the pinned typia 13.0.0-dev does not export (it is `createValidate`). 195 errored cases, so both `getvalidationerrors` pages have been shipping with no typia column. Same root cause as the todo above: `tsc` would have caught it instantly.
- [zod-bench-lane-permanently-reports-failed.md](../todos/zod-bench-lane-permanently-reports-failed.md) — three standing zod divergences make its lane print "FAILED" on every run, so a lane that genuinely breaks is indistinguishable. Observed in the same run: ajv's real build failure printed the identical line.
- [bench-one-aggregate-chokes-on-audit-results.md](../todos/bench-one-aggregate-chokes-on-audit-results.md) — `rtx bench --one <competitor>` crashes in `aggregate.mjs` when audit artifacts are present, because it assumes every `results/*.json` has a `cases` array. Hit while re-running ajv; worked around by invoking `gen-docs.mjs` directly.
