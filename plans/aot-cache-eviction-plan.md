...
- Mark `_used` on write paths that create or update entries during compile, so newly created routes/useFns don’t get dropped if they are never subsequently read.
