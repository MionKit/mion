...
// have already produced real validators). Gated behind MION_AUDIT_ALIGNMENT=1 via
...
/** Called from each competitor's main.ts. When MION_AUDIT_ALIGNMENT=1 it collects the
...
export function maybeAudit(name: string, cases: CompetitorModule['cases']): void {
  if (process.env.MION_AUDIT_ALIGNMENT !== '1') return;
