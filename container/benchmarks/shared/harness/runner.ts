...
const NO_TIMING = process.env.MION_VALIDATION_BENCH_NO_TIMING === '1';
const TIME_MS = Number(process.env.MION_VALIDATION_BENCH_TIME_MS ?? 100);
...
// MION_VALIDATION_BENCH_SKIP_GROUPS=<GROUP,GROUP>: record every case in these groups as
...
const SKIP_GROUPS = new Set(
  (process.env.MION_VALIDATION_BENCH_SKIP_GROUPS ?? '')
    .split(',')
  ...
  for (const iterated of iterateCases()) {
    ...
    if (SKIP_GROUPS.has(iterated.group.toUpperCase())) {
      const skipped = (): MetricResult => ({
        ...
        invalidOpsSec: 0,
        mixedOpsSec: 0,
        detail: `group ${iterated.group} skipped on this runtime (MION_VALIDATION_BENCH_SKIP_GROUPS)`,
      });
