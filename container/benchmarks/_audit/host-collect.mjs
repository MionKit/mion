...
//
// Each competitor runs with MION_AUDIT_ALIGNMENT=1 (see shared/harness/audit.ts →
...
//   MION_AUDIT_TSX=/path/to/tsx node _audit/host-collect.mjs [zod typebox ajv typia]
...
const TSX = process.env.MION_AUDIT_TSX ?? 'tsx';
const DEFAULT = ['zod', 'typebox', 'ajv', 'typia'];

const competitors = process.argv.slice(2).length > 0 ? process.argv.slice(2) : DEFAULT;
const auditEnv = {...process.env, MION_AUDIT_ALIGNMENT: '1', MION_VALIDATION_BENCH_RESULTS_DIR: RESULTS_DIR};
