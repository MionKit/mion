...
// by listing DATETIME in RT_BENCH_SKIP_GROUPS. When DATETIME IS in scope, a missing
...
const skipsDateTime = (process.env.RT_BENCH_SKIP_GROUPS ?? '')
  .split(',')
...
if (!skipsDateTime && typeof (globalThis as {Temporal?: unknown}).Temporal === 'undefined') {
  throw new Error(
    'Temporal global missing — the benchmarks require Node >= 26 (native Temporal, no polyfill). ' +
      'On a runtime without Temporal (e.g. Bun), run with RT_BENCH_SKIP_GROUPS=DATETIME.'
  );
}
