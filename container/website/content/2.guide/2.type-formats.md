...
::note
A `pattern` always needs `mockSamples`, the canonical valid values the mock generator can draw from (a bare regex with no samples is rejected). The samples must fit any length bounds the type declares, otherwise mocking fails with a clear error instead of producing an invalid value. The build checks every sample with the same regular expression engine your app runs on, so patterns using features like lookbehind are fully verified too, and a pattern with a syntax error fails the build instead of crashing at startup. Reusing a pattern across types? Register it once with [`registerFormatPattern`](/guide/pure-functions) and reference it by `typeof`. A registered pattern can also carry a `message`, and that message is what appears as the reported value in the validation error when the pattern fails.
::
