...
::note
A `pattern` does not need `mockSamples` anymore: when you declare none, the build generates a pool of valid values from the regex itself (100 by default, tune it with the `patternSampleCount` option). The pool is drawn fresh on every build; to make it reproducible, pass a literal seed where you create the mock (`createMockDataFn<T>(undefined, {mock: {seed: 42}})`), and the same seed then drives both the generated pool and the runtime picks (see [Mocking](/guide/mocking)). Declare your own samples when you want curated values, or when the build reports it cannot generate for a construct (lookbehind is the usual case) and asks you to. Declared samples always win, must fit any length bounds the type declares, and each one is checked against the regex at build time with the same regular expression engine your app runs on, so patterns using features like lookbehind are fully verified too, and a pattern with a syntax error fails the build instead of crashing at startup. A bare regex value like `/x/` is still rejected (spell it as `{source: '...'}`). Reusing a pattern across types? Register it once with [`registerFormatPattern`](/guide/pure-functions) and reference it by `typeof`. A registered pattern can also carry a `message`, and that message is what appears as the reported value in the validation error when the pattern fails.
::
