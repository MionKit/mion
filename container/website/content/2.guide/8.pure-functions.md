...
`mockSamples` are optional: leave them out and the build generates a pool of matching values from the regex (deterministic, so rebuilds produce the same values). Declare them when you want the [mock generator](/guide/mocking) to draw from curated values, and each declared sample is checked against the regex _at registration_. A sample that doesn't match throws immediately, so a broken pattern fails loud and early.
