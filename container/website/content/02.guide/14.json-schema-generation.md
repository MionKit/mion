...
`createJsonSchemaFn<T>()` returns a function that produces the document for `T`. It accepts the same four call forms as every other factory (type-first, value-first, run-type, JSON Schema literal).
