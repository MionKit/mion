...
::note
These are the same options a JSON Schema uses, under the same names. A schema with `uniqueItems` or `patternProperties` recovers exactly these types, so a shape can move between a schema, a type and a builder without changing what is enforced. The [JSON Schema guide](/guide/json-schema#json-schema-to-typescript) lists every keyword beside its two spellings.
::
