...
::note
The main goal of this extension is to store full JavaScript and TypeScript type information inside a JSON Schema. It is mainly used by the [type converter](/guide/converting-forms), which writes these keywords when it turns TypeScript files into schemas and reads them back when it converts in the other direction.
::
...
You rarely write these keywords by hand. The [convert command](/guide/converting-forms) writes them when it turns TypeScript files into schemas, and `runTypeFromJsonSchema` reads them back. If you need schemas with no extension at all, pass `--portable` to the convert command (or set `convertDialect: 'standard'` in your tsconfig): it refuses every extension keyword instead of writing one.
