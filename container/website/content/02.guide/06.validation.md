...
This page covers **validation**: the type guards, the error reports, and unknown-key handling. ([Serialization](/guide/serialization), [mocking](/guide/mocking) and [reflection](/guide/reflection) get their own pages.)
...
## Four ways to call any factory

Every factory takes your type four different ways. They all resolve to the same generated function, so use whichever fits the call site.
...
- **JSON Schema**: pass a draft 2020-12 literal; `T` is recovered from it. See [JSON Schema](/guide/json-schema).

The rest of these guide pages use whichever form reads best for the example, but every one of them accepts all four.
...
It takes the same four call forms as every other factory (type-first, value-first, run-type, JSON Schema) and the same `ValidateOptions`.
