...
description: Extra keywords that say what the JSON becomes in JavaScript.
toc: false
---

[Experimental]{.rt-badge-experimental} Part of the JSON Schema support; the surface may still change between minor versions.
...
::note
The main goal of this extension is to store full JavaScript and TypeScript type information inside a JSON Schema. Two features write it: the [schema generator](/guide/json-schema-generation), whose documents carry these keywords so a reader knows what the JSON decodes to, and the [type converter](/guide/source-conversion), which writes them when it turns TypeScript files into schemas and reads them back when it converts in the other direction.
::
...
You rarely write these keywords by hand. The [schema generator](/guide/json-schema-generation) emits them in the documents it produces, the [convert command](/guide/source-conversion) writes them when it turns TypeScript files into schemas, and `runTypeFromJsonSchema` reads them back. If you need schemas with no extension at all, pass `{portable: true}` to the generator or `--portable` to the convert command (or set `convertDialect: 'standard'` in your tsconfig): the generator strips every extension keyword, the converter refuses to write one.
...
The union row is how this library serializes unions that plain JSON can not tell apart. After a Date and a string member both encode, the wire holds just a string, so the encoder writes `[0, "2026-08-10T09:00:00Z"]` where the first slot says which member the value belongs to; object members share a single merged arm under index `-1`. Unions whose members are all plain JSON (`'a' | 'b'`, `string | null`) travel as their bare values and keep the ordinary `enum` or `anyOf` document with no `jsType`. A [generated document](/guide/json-schema-generation) always describes exactly the form that travels, so anything that validates against the document is exactly what the decoder accepts.
...
The formal version of this page, one precise rule per keyword, lives in the repository at [json-schema-2020-12-javascript.md](https://github.com/MionKit/ts-run-types/blob/main/docs/json-schema-2020-12-javascript.md); every rule there is backed by a test against the real convert command. To see the extension in action on your own types, run the [convert command](/guide/source-conversion).
