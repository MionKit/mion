...
::note
The main goal of this extension is to store full JavaScript and TypeScript type information inside a JSON Schema. The [schema generator](/guide/json-schema-generation) writes it: the documents it emits carry these keywords so a reader knows what the JSON decodes to.
::
...
You never write these keywords by hand. The [schema generator](/guide/json-schema-generation) emits them in the documents it produces. If you need schemas with no extension at all, pass `{portable: true}` to the generator and it strips every extension keyword.
