...
mion has three starter projects that set up a fully typed frontend and backend in one step. Each keeps the mion API in its own `api/` package, so you can deploy the frontend and the API independently.
...
::note
All starters compile validation and serialization at build time, for faster cold starts: the Vue and Nuxt starters with the [mion Vite plugin](../devtools/vite), the Next.js starter with [`withMion`](../devtools/nextjs). To deploy where runtime `new Function()` is blocked, set [`emitMode: 'both'`](/rpc/devtools/vite#plugin-options).
::
