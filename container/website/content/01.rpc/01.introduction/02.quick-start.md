...
::note
All starters use the [mion Vite plugin](../devtools/vite) to compile validation and serialization at build time, for faster cold starts. To deploy where runtime `new Function()` is blocked, set [`emitMode: 'both'`](../devtools/vite#plugin-options).
::
