...
- [mion Vite plugin](../devtools/vite) that compiles validation and serialization at build time
...
- mion Vite plugin wired through Nuxt's Vite config for build-time type compilation
...
- A catch-all route handler at `app/api/[...mion]/route.ts` serves the API, in dev and in production alike
- One app, one program, one deployment: no second server to start and no proxy
- Fully typed `@mionjs/client` usable from server and client components
- mion Vite plugin for build-time type compilation
...
::note
All starters wire in the [mion Vite plugin](../devtools/vite) so validation and serialization functions are compiled at build time — enabling deployment to locked-down environments (no runtime `new Function()`) and faster cold starts.
::
