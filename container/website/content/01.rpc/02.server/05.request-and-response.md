...
The [MionRequest](#mionrequest) and [MionResponse](#mionresponse) objects are available to every middleFn/route within the [`CallContext`](/rpc/server/routes) (the first parameter of every middleFn/route).
...
**Example:**
[`@mionjs/platform-node`](/rpc/platforms/node-js) uses node's native `IncomingMessage` and `ServerResponse` while [`@mionjs/platform-aws`](/rpc/platforms/aws-lambda) uses `APIGatewayEvent` and `APIGatewayProxyResult`. But routes and middleFns will always receive a standard MionRequest & MionResponse.
...
::tip
The underlying logic to format request & parse response data is handled transparently by the [Client](/rpc/client/client-overview).
...
For more details on error handling patterns, see [Error Handling](/rpc/server/error-handling).
