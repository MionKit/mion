...
| `markers` | unset | Which packages may declare the RunTypes marker types, an object detailed in the table below. Leave the key out and only `@ts-runtypes/core` is trusted, which is what almost every project wants. It exists for a library that wants to expose RunTypes powered functions without asking its own users to install RunTypes for the type declarations alone. |
| `convertDialect` | `extended` | Which JSON Schema the [convert command](/guide/source-conversion) writes when it moves a file to schema form. `extended` uses the RunTypes dialect, so types with no standard spelling (a `Date`, a `Map`, a `bigint`) survive the trip. `standard` writes plain JSON Schema 2020-12 only, and a declaration that needs the dialect becomes an error instead of losing its meaning. Convert is a command line tool, so this key is tsconfig only; `--portable` is the same switch for a single run. |
...
| Key | Default | What it does |
| --- | --- | --- |
| `packages` | none | Extra package names allowed to declare the marker types. This adds to the built in package, it never replaces it, so markers imported from `@ts-runtypes/core` keep working alongside your own. |
...
Re-exporting the marker types from your own package (`export type {InjectRunTypeId} from '@ts-runtypes/core'`) needs no configuration at all, because the declaration still belongs to RunTypes. You only need this key when your package writes its own declarations.
...
| `jsRuntime` | Path to the JavaScript runtime the compiler runs regex pattern checks on (node, bun, or any compatible runtime). Defaults to the same runtime your build already uses, so it almost never needs setting. |
