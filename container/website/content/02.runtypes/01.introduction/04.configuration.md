...
::note
mion adds one key of its own to the same plugin entry, `clientTsconfig` (CLI flag `--client-tsconfig`): the tsconfig of a separate client project a mion API serves batches to. It is documented with the [mion Vite plugin](/rpc/devtools/vite#batch-transport).
::
