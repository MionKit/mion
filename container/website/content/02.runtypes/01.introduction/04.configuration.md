...
| `hashLength` | `7` | Length of the short ids in generated names. Every id is exactly this long. If two types ever land on the same id, the build stops and names both types, the id they share, and the length to try next (see [MKR014](/runtypes/diagnostics/all-diagnostics)). That is the only reason to raise it, and one more character is sixty-two times the room. |
| `downgradeErrors` | unset | RuntimeError findings (a type that can never be encoded, a contradictory format rule) fail your build: your bundler produces no bundle, and the `mion` command line exits with a failure code. See [Error Levels](/runtypes/diagnostics/error-levels) for the three levels and what each lane does. List the codes you want reported as warnings instead, for example `['VL002']`, and every other finding keeps stopping the build. The downgraded finding is still printed, marked `(downgraded)`. Use `'*'` to downgrade the lot, which suits a project adopting mion that cannot list codes it has not met yet. An Error is out of reach, `'*'` included, because the build produced no code for that piece, so naming one here is refused with the reason. For a bad call site in your own source prefer a `@mion-expect-error` comment on the line above it: it is precise, and an unused one is reported as a warning, so it cannot outlive the problem. |
| `pureFnReport` | off | Emit a structured report of every pure function this build generated, for tooling that moves pure function bodies between two builds of one codebase (a framework that declares a mapper in client code but runs it on the server). On the bundler plugin it is one switch with three states: `'file'` writes the report to `<genDir>/types/pure-fns-report.json` on every build (in the generated folder, so it is gitignored and rewritten like the rest of the output); `'callback'` delivers it only in process to the `onPureFnReport` handler, writing no file; leave it off (the default) for neither. The file location is fixed (like every path under `genDir`), so it is not configurable. Each record carries the call site, the pure function's id, which function and package declared the registrar (so a framework wrapper is attributed to the framework, not to `@mionjs/run-types`), and the function body itself. The report shape is the same whatever `moduleMode` you use. In tsconfig, set `pureFnReport: true` to write the file (the in process handler is a plugin only API); the CLI flags are `--pure-fn-report-file` (writes the JSON) and `--pure-fn-report-wire` (report on the wire only). |
| `jsonMaxBytes` | `true` | Emit, for every type a marker reflects whose size is fully bounded (strings with a maximum length, lists with a maximum number of items, and so on), the largest JSON a valid value can be. A framework turns that number into request size limits per route, which is what mion does. Set it to `false` and nothing is emitted, for a project that keeps its own limits. The CLI flag is `--json-max-bytes=false`. |
