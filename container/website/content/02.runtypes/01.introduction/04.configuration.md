...
| `downgradeErrors` | unset | Error findings (a type that can never be encoded, a contradictory format rule) stop the build in every lane, including tests and the dev server, just like a type error would. List the codes you want reported as warnings instead, for example `['VL002']`, and every other error keeps stopping the build. The downgraded finding is still printed, marked `(downgraded)`. Use `'*'` to downgrade the lot, which suits a project adopting mion that cannot list codes it has not met yet. For a bad call site in your own source prefer a `@mion-expect-error` comment on the line above it: it is precise, and an unused one is reported, so it cannot outlive the problem. See [Diagnostics](/runtypes/diagnostics). |
