...
A Map's key and value types are read from the schema itself, as the two `prefixItems`; a Set's item is `items`. There is no separate argument list, because the schema already had to describe the same JSON. A Set or Map with a size limit (a `FormattedSet` or `FormattedMap`) carries `minItems` / `maxItems` on that outer array, and a Set's `contains` rides the standard keyword too.
