...
A Map's key and value types are read from the schema itself, as the two `prefixItems`; a Set's item is `items`. There is no separate argument list, because the schema already had to describe the same JSON. A Set or Map with constraints (a `FormattedSet` or `FormattedMap`) carries them on that outer array, under the standard keyword names: `minItems` / `maxItems`, `uniqueItems`, and `contains`. A Map's entry is its pair, so its `contains` prints as a two-slot array schema.
