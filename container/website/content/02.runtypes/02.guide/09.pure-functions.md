...
Register a pure helper with `registerPureFnFactory(factory)`. The factory returns the real function, the build inlines it into the generated code, and what you get back is the helper's **id**: where it lives, built from your package, the file, and the name you bound it to.
...
::tip
A helper you do not assign to a name (a callback handed straight to a library) is identified by its body instead of its location, so the same helper written twice is one entry.
::
