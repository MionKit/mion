...
It mirrors the named lane's two forms. `registerAnonymousPureFn` takes the function directly, which is the shape a single-callback API like `inputFrom(order, o => o.id)` wants. When the helper needs one-time setup or wants to compose another pure fn, reach for `registerAnonymousPureFnFactory` and pass a factory instead.
