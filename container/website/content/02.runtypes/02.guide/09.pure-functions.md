...
## How a Helper Is Identified

An id is built from the helper and nothing else:

```ts
export const slugify = registerPureFnFactory(...);
// @acme/text#9Zt1bRm4cVaPqL
```

That gives you three things worth knowing.

Renaming the helper, renaming what you export it as, or moving it to another file inside the same package all leave the id exactly as it was. Editing the helper, or editing anything it uses, gives it a new id, because it is a new helper. And the same helper written twice is one entry, so nothing is compiled or shipped twice.

::tip
Because the id is built from the helper, it can never point at the wrong one. A reference to an id that no longer exists fails loudly at build time instead of quietly resolving to something else.
::
