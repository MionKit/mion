...
::note
**Validators check serializable data.** `createValidateFn`, `createGetValidationErrorsFn`, and the JSON and binary codecs all work on the data-only projection of your type, `DataOnly<T>`: the JSON-shaped data your value carries. Non-serializable members (functions, methods, getters, symbols and `RegExp` values) are skipped, along with built-in values that have no JSON form, such as `URL` or a typed array. None of them survive a round trip through JSON or the wire anyway, and you get a build warning naming each one that is dropped. A value that is not serializable at a root position has nothing to validate, so the generated function is written as one that throws and the build reports it as a RuntimeError. This is why a validator can flag a type as non-serializable, and why a decoder returns `DataOnly<T>` rather than the full type.
::
...
Working from a JSON payload rather than a value you already hold? [createParseFn](/runtypes/guide/json-serialization#parsing-and-validating-in-one-step) does the decoding and the checking together, and takes the same strategy choice.
