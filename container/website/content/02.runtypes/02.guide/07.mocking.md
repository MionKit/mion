...
Mock generation has its own import, so an app that never uses it ships none of it.
...
::note
`createMockDataFn` needs the Vite plugin running to read your type. It throws a clear error if the plugin isn't active. It's a build-time tool, so use it in tests and seed scripts, not shipped runtime code.
::
