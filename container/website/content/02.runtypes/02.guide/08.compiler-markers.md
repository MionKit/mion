...
The keys mirror the encoder and decoder strategies: `pjs` and `pj` are the clone and mutate prepares, `rjs` and `rj` the matching restores (`rjs` drops undeclared keys, `rj` keeps them), `cj` and `cjr` are the compact encode and decode, and `sj` is the single-pass stringify. This is what a framework reaches for when it owns its own JSON envelope: parse one request body once and restore each value, prepare each value and stringify one response once, with no extra string round-trip in between.
