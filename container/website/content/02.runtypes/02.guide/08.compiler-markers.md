...
fn: PureFunction<F>
```
Brands a pure function argument that passes the purity rules. The direct form: the argument is the helper itself. Used for [pure functions](/runtypes/guide/pure-functions).
:::
...
```
The factory form of the above, for a helper that needs one-time setup or composes another pure fn. Used for [pure functions](/runtypes/guide/pure-functions).
:::
...
```
Injects a content hash of a pure helper's body, so a library can wrap `registerAnonymousPureFn`. Used for [pure functions](/runtypes/guide/pure-functions).
:::
...
Each name says what the function does, and they mirror the encoder and decoder strategies.

| Name | What it does |
| --- | --- |
| `prepareForJsonClone` | Builds a new JSON-safe value, so undeclared keys are dropped |
| `prepareForJsonMutate` | Transforms the value in place, so undeclared keys are kept |
| `restoreFromJsonStrip` | Rebuilds the declared shape, so undeclared keys are dropped |
| `restoreFromJson` | Restores in place, so undeclared keys are kept |
| `compactForJson` / `compactFromJson` | The compact encode and decode |
| `stringifyJson` | Value to JSON string in a single pass |

This is what a framework reaches for when it owns its own JSON envelope: parse one request body once and restore each value, prepare each value and stringify one response once, with no extra string round-trip in between.
...
The value it returns is stable across releases, so it stays valid when you upgrade. It also follows compile-time options, so `getFnHash('validate', {noLiterals: true})` and `getFnHash('jsonEncoder', {strategy: 'mutate'})` return the ids for those specific variants, not just for the family.
