...
A `pattern` is also checked for runaway backtracking before the build finishes. Some regular expressions can match the same text more than one way, and on an input that almost matches, JavaScript tries every combination in turn. A pattern like `^(\w+\s?)*$` is one of those: a few dozen characters are enough to freeze the validator, which is the thing meant to keep bad input out. The build rejects such a pattern and quotes the part responsible, so it never reaches your users. This check reads the pattern itself rather than timing it, so it behaves the same on every machine and under every JavaScript runtime.
