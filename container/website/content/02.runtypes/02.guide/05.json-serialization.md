...
Each one compiles to a different specialized function. The first three write the same JSON and differ only in the work behind it; `compact` writes a smaller document:
...
| `compact`           | Like `clone`, but every object is written as a positional array with no key names, which makes the JSON 40 to 60 percent smaller on real world data. Both ends must share the type, the way the binary codec does, and an absent optional property rides as `null`. | No             | Dropped            |

`createJsonDecoderFn` takes a `strategy` too: `strip` (the default) drops undeclared keys, `preserve` keeps them, and `compact` reads the positional arrays the `compact` encoder writes. The key based decoders cannot read a compact document, so the two ends of a compact wire always pick it together.

::tip
`clone` is the safe default. It never touches your input and drops stray keys by building a fresh value from the type. Reach for `mutate` only in a hot path where the allocation shows up in a profile, and you're fine mutating the object you pass in. Reach for `compact` when the payload size is what matters and you control both ends.
::
...
- `deserialize` defaults to `new YourClass()` followed by setting the class's declared properties from the decoded data, so a class with a zero-argument constructor needs nothing but the class. The rebuild is driven by the type: a key the class does not declare is never copied onto the instance. Provide `deserialize` when the constructor takes arguments (the type checker requires it there).
