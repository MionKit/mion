...
`createPrepareForJsonFn` turns a typed value into a JSON-safe value, and `createRestoreFromJsonFn` turns it back. The string encoder and decoder are built on these. Both take the same `strategy`, so pair `clone` with `clone`.
