...
Every value is rebuilt only from the form the encoder writes: a `Date` or a `bigint` from its string, a `Map` or a `Set` from its array. Anything else at that position is refused on the spot, so a `null` never turns into an epoch date and a `true` never turns into `1n`. The plain decoders follow the same rule and throw a regular `Error` whose message names the type and the form it expected, for example `[mion] Can not json decode Date: expected an ISO date string`; inside `parse` that message becomes the `deserializeError` of the `RTParseError`.
