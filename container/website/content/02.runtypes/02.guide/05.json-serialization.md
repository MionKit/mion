...
Each one compiles to a different specialized function. The first three write the same keyed JSON, `compact` writes a smaller positional form:
...
| `compact`           | Like `clone`, but every object is written as an array of its declared properties, in order, with no key names on the wire. 40 to 60 percent fewer bytes on real objects. An absent optional property rides a `null` placeholder. | No             | Dropped            |

The decoder has a `strategy` too. `strip` and `preserve` read the keyed JSON the first three write and differ only in what happens to undeclared keys (see [Parsing and Validating in One Step](#parsing-and-validating-in-one-step)). `compact` reads the positional form, so pair it with the `compact` encoder; the keyed decoders cannot read it.

::tip
`clone` is the safe default. It never touches your input and drops stray keys by building a fresh value from the type. Reach for `mutate` only in a hot path where the allocation shows up in a profile, and you're fine mutating the object you pass in. Reach for `compact` when both ends share the type and the bytes matter, the way a mion route does.
::

## Custom Class Serializers

Plain objects are handled for you, and the built-in classes (`Date`, `Map`, `Set`) round-trip for free. A `RegExp` does not: a pattern is code, not data, so a `RegExp` property is dropped from the wire like a function is. Your own class is different. A plain object is pure data: everything it carries survives JSON and comes back the same. A class instance is not. Its methods and getters live on the prototype (they are code, so they never ride the wire), its constructor runs real logic, and it may hold values that cannot be serialized at all. So the wire can only carry the data of an instance; turning that data back into a live object needs code the build tool does not have. That is what you register once with **`registerClassSerializer`**: you hand it the class itself and (when needed) how to rebuild an instance.
...
- `deserialize` defaults to `new YourClass()` followed by setting the class's declared properties from the decoded data, so a class with a zero-argument constructor needs nothing but the class. Provide `deserialize` when the constructor takes arguments (the type checker requires it there).
...
A subclass is a class of its own. To send one, declare it in the signature next to its base (`AuthError | RpcError`) and register it like any class: the union picks the arm by the exact class, so an `AuthError` rides its own arm with every field it declares and comes back as an `AuthError`, whatever order the members are listed in. Left unregistered, it still takes its own arm by shape and comes back as a plain object. Handing a subclass to an encoder built for the base alone is a type error, not a supported path.
...
::note
The same `rejectCircularRefs` option is available on the [binary encoder](/runtypes/guide/binary-serialization#detecting-circular-references), on `createValidateFn` and on `createGetValidationErrorsFn`. In validation, `validate` returns `false` and `getValidationErrors` records a `{expected: 'circular'}` entry instead of throwing. Decoders need no guard: a serialized payload can't contain a runtime cycle. The whole guard is off by default; arm it only where you process values that might cycle.
::
...
What a decoder checks on its own, what it leaves for the check, and what it throws is spelled out on the [Decoding Untrusted Input](/runtypes/guide/decoding-untrusted-input) page.
...
This is on purpose, and the build tells you when it happens with a `VL010`-family **Warning**, which never stops anything. (At a position that would throw at runtime, like a union member or array element, it is escalated to a **RuntimeError**: the function is still written, it just throws when you call it, and the build fails. See [Diagnostics](/runtypes/diagnostics/error-levels).) The reasoning is in [About RunTypes](/runtypes/introduction/built-on-typescript-go#extending-typescript-go-without-modifying-it).
...
`createPrepareForJsonFn` turns a typed value into a JSON-safe value, and `createRestoreFromJsonFn` turns it back. The string encoder and decoder are built on exactly these. Both take the same `strategy`, so pair `clone` with `clone`.
