...
This is on purpose, and the build tells you when it happens with a `VL010`-family **Warning**, which never stops anything. (At a position that would throw at runtime, like a union member or array element, it is escalated to a **RuntimeError**: the function is still written, it just throws when you call it, and the build fails. See [Diagnostics](/runtypes/diagnostics/error-levels#the-three-levels).) The reasoning is in [About RunTypes](/runtypes/introduction/built-on-typescript-go#extending-typescript-go-without-modifying-it).
