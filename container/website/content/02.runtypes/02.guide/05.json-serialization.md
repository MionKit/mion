...
A subclass answered where its base is declared is encoded as the declared class. An `AuthError extends RpcError` returned from a handler typed `RpcError` takes the structural road: the fields `RpcError` declares go on the wire and an `RpcError` comes back. Fields the subclass added are undeclared keys, and they follow the same rules as an undeclared key on a plain object: the `mutate` encoder leaves them in place, `clone` and `compact` build the declared shape without them, and the decoder's `strip` removes what is left. To carry them, declare the subclass in the signature and register it.
