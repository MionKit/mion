...
A subclass of a registered class needs no registration of its own. An `AuthError extends RpcError` answered where `RpcError` is declared is still an `RpcError`, so it takes that class's lane, and the fields the subclass added ride along: they go on the wire next to the declared ones and come back on the rebuilt instance. What comes back is an instance of the declared class, not of the subclass, and the added fields are carried as data, not validated. On decode the JSON decoder's `strategy` still applies: the default `strip` removes undeclared keys before the rebuild, `preserve` keeps them. A key named `__proto__`, `constructor` or `prototype` is never copied.
