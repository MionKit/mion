...
::note
These run at build time. With the Vite plugin off there's no id to inject, so `getRunTypeId` throws rather than guess. The id is filled in by a [compiler marker](/runtypes/guide/compiler-markers).
::

## Getting a RunType Node

`getRunType<T>()` returns the node for a type. Pass the type, or pass a value and let it be inferred. The [compiler markers](/runtypes/guide/compiler-markers) fill in the id at build time, so the call just works.
...
Around them you will find `name` and `optional` on members, `literal` and `values` for literals and enums, and `formatAnnotation` for a branded [TypeFormat](/runtypes/guide/type-formats) like an email or a bounded date.
