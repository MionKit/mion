...
| `patternSampleCount` | `100` | How many mock samples the build generates for a string `pattern` that declares no `mockSamples` of its own. The values are drawn from the regex itself, fresh on every build; a literal seed on `createMockDataFn` pins them (see [Mocking](/guide/mocking)). Set `0` to turn generation off; sample-less patterns then fail the build asking you to declare samples. The CLI flag is `--pattern-sample-count`. |
| `patternSampleRetries` | `10` | How many draws the generator may spend per sample before giving up (the whole budget is the count times this). Raise it for a heavily constrained pattern whose candidates often miss the declared length bounds. The CLI flag is `--pattern-sample-retries`. |
...
| `jsRuntime` | Path to the node or bun the compiler runs regex pattern checks on. Defaults to the same runtime your build already uses, so it almost never needs setting. |
