...
| `binary` | Points the plugin at a custom compiler build, used when developing RunTypes itself. The `RT_BIN` environment variable (below) does the same for every tool at once. |
...
## Choosing the compiler binary

RunTypes installs a small prebuilt compiler for your platform and finds it automatically, so there is normally nothing to configure. Set the `RT_BIN` environment variable when you need a specific build instead:

```bash
RT_BIN=/path/to/ts-runtypes pnpm run build
```

The path can be absolute or relative to the directory you run the command in, and it must point at an executable file. A path that does not resolve stops the run with a clear message rather than quietly falling back to the installed compiler.

Unlike the `binary` option above, which only covers the bundler plugin, `RT_BIN` applies to every tool that runs the compiler, including the [lint plugin](/guide/linting). That makes it the way to try a build you have not published yet, to check whether an older compiler behaves differently, or to run a compiler that reached the machine outside the package manager.

::note
The compiler version is part of every generated type id, so a build of a different version writes cache entries that do not match a normal install. Clear `node_modules/.cache/ts-runtypes` when you go back.
::
