...
| `patternSampleCount` | `100` | How many mock samples the build generates for a string `pattern` that declares no `mockSamples` of its own. The values are drawn from the regex itself and come out the same on every rebuild. Set `0` to turn generation off; sample-less patterns then fail the build asking you to declare samples. The CLI flag is `--pattern-sample-count`. |
