...
pnpm add -D mion run-types/devtools
```
```bash [npm]
npm install @mionjs/run-types
npm install -D mion run-types/devtools
```
```bash [yarn]
yarn add @mionjs/run-types
yarn add -D mion run-types/devtools
```
::

::note
RunTypes is the only thing that ends up in your runtime bundle, and it carries no dependencies. `mion run-types/devtools` does its work at build time and ships nothing to production.
::
...
| Bundler | Import |
| --- | --- |
| Vite | `mion run-types/devtools/vite` |
| Rollup | `mion run-types/devtools/rollup` |
| Rolldown | `mion run-types/devtools/rolldown` |
| webpack | `mion run-types/devtools/webpack` |
| Rspack | `mion run-types/devtools/rspack` |
| esbuild | `mion run-types/devtools/esbuild` |
| Bun | `mion run-types/devtools/bun` |
| Next.js | `mion run-types/devtools/next` |
...
```ts
import runtypes from 'mion run-types/devtools/bun';
...
The rewrite is identical whatever bundler runs it, so your generated code does not change when you switch. The Vite entry adds hot reload on top, and for a tool that is not on the list, `mion run-types/devtools/unplugin` exposes the framework-agnostic [unplugin](https://github.com/unjs/unplugin) instance the others are built from.
