...
something different about each. Those set a `type` on the `format` detail
naming the mode, so you can switch on it:

```ts
const errors = getErrors(card);
switch (errors[0]?.format?.type) {
...
A format with a single failure mode leaves `type` unset, so its absence means
there was only one thing that could have gone wrong.
