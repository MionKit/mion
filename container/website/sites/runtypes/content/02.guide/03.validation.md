...
### Unions close each branch on its own

On a union, `checkUnknowns` follows the branch the value matched, so a property that belongs to a different branch counts as extra. `createHasUnknownKeysFn` cannot do that, because it never validates and so never learns which branch matched. It pools every branch's property names into one list instead, which makes it more forgiving here:

<code-import path="packages/examples/src/guide/unknown-keys-unions.ts" lang="ts" />

A key that belongs to no branch at all is rejected by both. Where the two need to agree, use `checkUnknowns`. On the error form a union reports `{path: [], expected: 'union'}`, because the offending key is only extra relative to a branch.
