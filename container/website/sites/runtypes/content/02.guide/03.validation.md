...
::note
**Validators check serializable data.** `createValidateFn`, `createGetValidationErrorsFn`, and the JSON and binary codecs all work on the data-only projection of your type, `DataOnly<T>`: the JSON-shaped data your value carries. Non-serializable members (functions, methods, getters and symbols) are skipped, since they never survive a round trip through JSON or the wire anyway. A value that is not serializable at a root position has nothing to validate, so the build reports it. This is why a validator can flag a type as non-serializable, and why a decoder returns `DataOnly<T>` rather than the full type.
::

## Standard library types are not expanded

A type that comes from the TypeScript standard library is treated as a whole value, never taken apart into its members. `URL`, `Blob`, `Headers`, `Intl.DateTimeFormat`, the iterator types, `ArrayBuffer` and the typed arrays all fall under this. None of them has an agreed JSON form, so there is nothing to check field by field.

Five have a form everyone agrees on, and those are supported as data: `Date`, `RegExp`, `Map`, `Set` and the Temporal types.

You always hear about it. A property holding one of these raises a build warning naming the property, and the rest of the object still validates:

```
warning VL015: Property `link` has a non-serialisable value type: `validate` drops it,
so this property is silently not validated.
```

One of these at a root position is an error instead, because nothing would be left to check.

The fix is to describe the data you actually send:
