...
After `validate`, opt the predicate into the key-count fast path. It applies at every depth, so nested objects get it too, whether they are written inline or named as their own type. It keeps the array rule above: an array can pass an object type it happens to satisfy, and an array has no undeclared properties, so the answer is still false.
