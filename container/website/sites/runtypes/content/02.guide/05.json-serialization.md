...
It takes the output of `JSON.parse`, not the string, so it drops in wherever the body was already read. A value that matches comes back typed, with dates and bigints turned back into real values. One that doesn't throws `RTParseError`, and its `issues` are exactly the entries `createGetValidationErrorsFn` produces, so anything already rendering those needs no second path.
...
Most projects want the same answer everywhere rather than a choice at each call. Set it once under a `parse` object in the plugin (or the tsconfig `ts-runtypes` plugin entry) and every parser picks it up:

```ts
runtypes({parse: {strategy: 'strip'}})
```

A per-call `strategy` still wins over the project default for that one parser, and setting `strategy: 'preserve'` at a call opts it back out.
