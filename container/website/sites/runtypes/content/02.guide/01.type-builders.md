...
This pattern also keeps your bundle small. A schema you only take the type of ships no runtime description at all: the build sees that the value is never used and generates just the functions you asked for. Handing the schema to a create function is free too, since the function it builds carries everything it needs. What does keep the runtime description is reading the schema itself, so exporting it or touching its properties. When other files only need the shape, export the type rather than the schema.
