...
`TF.CreditCard` (builder `TF.creditCard()`) checks a payment card number. It runs the Luhn checksum, so a single mistyped digit is caught rather than waved through by a length check. Spaces and dashes between digits are accepted out of the box, since that is how a card number is printed and typed. Accepting them does not rewrite them: add `stripSeparators` when you want bare digits back, and only the format transform applies it, never validation. Pass `networks` to narrow it to the issuers a field accepts, or `separators` to change which characters are allowed (an empty string means digits only). Leaving `networks` out keeps the card-network table out of your bundle entirely. Mock data builds a fresh valid number each draw, with a real prefix for its network; set the `testCreditCards` mock option when the data reaches a payment sandbox, which only accepts the published test numbers.
