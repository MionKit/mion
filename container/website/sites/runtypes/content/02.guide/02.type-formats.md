...
`TF.CreditCard` (builder `TF.creditCard()`) checks a payment card number. It runs the Luhn checksum, so a single mistyped digit is caught rather than waved through by a length check. Pass `networks` to narrow it to the issuers a field accepts, and `separators` to allow the grouping people type in, such as `4111 1111 1111 1111`. Both are optional, and leaving `networks` out keeps the card-network table out of your bundle entirely.
