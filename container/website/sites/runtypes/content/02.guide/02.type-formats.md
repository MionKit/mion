...
::note
The fixed-width number/bigint formats (`int32`, `bigUInt64`, …) aren't just validation. They also tell the [binary codec](/guide/binary-serialization) how many bytes to pack. A `TF.UInt8` field rides the wire in a single byte.
::
...
- A mion route with `sanitizeParams` applies it to the request params after decoding and before validation, on the server and in the client. See [Sanitize params](https://mion.pages.dev/server/validation#sanitize-params) on the mion site.
