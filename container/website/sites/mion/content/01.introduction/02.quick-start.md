...
- [mion Vite plugin](../devtools/vite-config) that compiles validation and serialization at build time
...
::note
All starters wire in the [mion Vite plugin](../devtools/vite-config) so validation and serialization functions are compiled at build time — enabling deployment to locked-down environments (no runtime `new Function()`) and faster cold starts.
::
