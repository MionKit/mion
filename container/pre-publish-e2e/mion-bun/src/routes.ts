...
import {PublicApi, Routes, route} from '@mionjs/router';
...
export const routes = {
    sayHello: route((_ctx, user: SimpleUser): string => `Hello ${user.name}`),
    calculateAge: route((_ctx, birthYear: number): number => 2026 - birthYear),
