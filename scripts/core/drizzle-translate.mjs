...
function writeTsconfig(workDir) {
  ...
  const config = {
    ...
    compilerOptions: {
      target: 'ESNext',
      // Pinned like the container lane's tsconfig, and for the same reason:
      // on the ESNext lib Node's Buffer iterator types resolve to
      // IteratorObject, which reflection refuses (MKR009), so the type-road
      // tree cannot be built at all.
      lib: ['ES2023'],
      module: 'Preserve',
