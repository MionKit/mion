...
export const REGISTRY = [
  ...
  {name: 'MION_FUZZ_SECBINARY_SOAK_MS', scope: 'dev', task: '-', desc: 'security fuzz (binary decoder bytes) soak duration in ms'},
  ...
  {name: 'MION_TEST_PORT', scope: 'internal', task: '-', desc: 'Default port the mion test server entry listens on when the caller names none (read by packages/test-server/src/test-server.ts and the e2e mion consumer server; defaults to 8076). Nothing is spawned: the client tests pass their port to startTestServer() directly'},
  {name: 'MION_TEST_SERVER_AUTO_START', scope: 'internal', task: '-', desc: "Set to 'true' to make the test-server entry start a server when the module is IMPORTED. Opt-in: importing it never starts one otherwise. Used by the compiled-server e2e lane and by the middleware-mode e2e spec, which loads the entry through a vite dev server"},
  ...
  {name: 'MION_E2E_BUNDLE_API', scope: 'internal', task: '-', desc: "bundleApi mode apps/mion-next's next.config.mjs builds on, set by build-all.mjs for its SECOND Turbopack build. Unset is the fetched lane (the round trip the app serves); 'bundled' builds into dist/next-bundled (its genDir stays a sibling, .rt-bundled, because a genDir is a build INPUT), which test/build-outputs.test.mjs reads to prove the fetched lane left the output"},
  ...
  {name: 'MION_E2E_MION_VERSION', scope: 'internal', task: '-', desc: '@mionjs/* version the mion consumer lanes install. Read from the framework packages\' own package.json files (which must agree with each other and with version.json), passed into the container via -e by scripts/release/e2e.mjs'},
  {name: 'MION_E2E_MATRIX_MION_PKGS', scope: 'internal', task: '-', desc: 'Space-separated <name>@<version> install list the e2e feature MATRIX adds on top of the type-system packages: what apps/mion-next imports (@mionjs/core, router, client, platform-vercel) at MION_E2E_MION_VERSION. Separate from MION_E2E_MION_PKGS because the matrix hosts one mion app, not the whole consumer surface (passed into the container via -e by scripts/release/e2e.mjs)'},
  {name: 'MION_E2E_MION_PKGS', scope: 'internal', task: '-', desc: 'Space-separated <name>@<version> install list for the mion consumer lane: the lockstep @mionjs/* packages at MION_E2E_MION_VERSION plus @mionjs/drizzle-orm and the @mionjs/drizzle-orm-*-core packages at their own drizzle-aligned versions (passed into the container via -e by scripts/release/e2e.mjs)'},
