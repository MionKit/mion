...
export const REGISTRY = [
  ...
  {name: 'RT_VALIDATION_BENCH_USE_LOCAL', scope: 'dev', task: '-', desc: 'Build the shared image locally for benchmark runs'},
  ...
  {name: 'RT_VALIDATION_BENCH_ENGINE', scope: 'dev', task: '-', desc: 'Container engine (default podman)'},
  {name: 'RT_VALIDATION_BENCH_IMAGE', scope: 'dev', task: '-', desc: 'Local image tag (default tsrt-website:dev)'},
  {name: 'RT_VALIDATION_BENCH_CONTAINER', scope: 'dev', task: '-', desc: 'Container name prefix (default tsrt-bench)'},
  {name: 'RT_VALIDATION_BENCH_NO_TYPIA', scope: 'dev', task: '-', desc: 'Skip the typia competitor (its native plugin build)'},
  {name: 'RT_VALIDATION_BENCH_QUICK', scope: 'dev', task: '-', desc: 'Fast/preview benchmark numbers (noisy)'},
  {name: 'RT_VALIDATION_BENCH_NO_TIMING', scope: 'dev', task: '-', desc: 'Correctness-only run (no timing)'},
  {name: 'RT_VALIDATION_BENCH_TIME_MS', scope: 'dev', task: '-', desc: 'Per-cell timing window in ms (default 100)'},
  {name: 'RT_VALIDATION_BENCH_BUN', scope: 'dev', task: '-', desc: "Set to 0 to skip the benchmarks' bun runtime lane (default: on). The lane re-runs each competitor's already-built dist/run.mjs under bun, which is what exercises the JavaScriptCore branch of rt::countEnumKeys"},
  {name: 'RT_VALIDATION_BENCH_SKIP_GROUPS', scope: 'internal', task: '-', desc: 'Comma-separated case GROUPS recorded as not-supported instead of run, for runtime capability gaps (the bun lane sets DATETIME: bun ships no Temporal). Set by bench.mjs; listed in each result JSON as skippedGroups'},
  {name: 'RT_VALIDATION_BENCH_CASE', scope: 'dev', task: '-', desc: 'Restrict a run to matching case names (inspection)'},
  {name: 'RT_VALIDATION_BENCH_DUMP', scope: 'dev', task: '-', desc: 'Print typecost probe sources (debug)'},
  {name: 'RT_VALIDATION_BENCH_SERIALIZATION_OUT', scope: 'dev', task: '-', desc: 'Serialization bench output dir (default container/website/public/bench-data)'},
  ...
  {name: 'RT_VALIDATION_BENCH_DOCDATA', scope: 'dev', task: '-', desc: 'Host dir to publish benchmark JSON into (default .docdata)'},
  {name: 'RT_VALIDATION_BENCH_REMOTE_IMAGE', scope: 'dev', task: '-', desc: 'GHCR ref to pull (default ghcr.io/$GHCR_OWNER/tsrt-website:latest)'},
  {name: 'RT_VALIDATION_BENCH_MOUNT_OPTS', scope: 'dev', task: '-', desc: 'Extra bind-mount opts, e.g. ":z" on SELinux'},
  {name: 'RT_VALIDATION_BENCH_RUN_NETWORK', scope: 'dev', task: '-', desc: 'podman run network (e.g. "host" behind a proxy)'},
  {name: 'RT_VALIDATION_BENCH_BUILD_NETWORK', scope: 'dev', task: '-', desc: 'podman build network, forwarded to the image build'},
  {name: 'RT_VALIDATION_BENCH_BASE_IMAGE', scope: 'dev', task: '-', desc: 'Node base image, forwarded to the image build'},
  {name: 'RT_VALIDATION_BENCH_PNPM_VERSION', scope: 'dev', task: '-', desc: 'pnpm version, forwarded to the image build'},
  {name: 'RT_VALIDATION_BENCH_CA_CERT', scope: 'dev', task: '-', desc: 'Extra CA certs, forwarded to the image build'},

  // — engine-branch tripwire (scripts/website/bench-data/engine-perf-check.mjs) —
  {name: 'RT_VALIDATION_BENCH_ENGINE_ASSERT', scope: 'dev', task: '-', desc: "Set to 1 to make the engine-perf check FAIL on an inverted rt::countEnumKeys counter instead of only reporting it (website-deploy.yml sets 0 while the arm64 numbers are unmeasured). NOT the same knob as RT_VALIDATION_BENCH_ENGINE, which picks the container engine"},
  {name: 'RT_VALIDATION_BENCH_ENGINE_MARGIN', scope: 'dev', task: '-', desc: 'How much faster the selected counter must be before the engine-perf check calls the pick wrong (default 1.15)'},
  {name: 'RT_VALIDATION_BENCH_ENGINE_ITERS', scope: 'dev', task: '-', desc: 'Iterations per engine-perf-check measurement (default 2000000)'},

  // — mion HTTP server benchmarks (`rtx bench servers`, scripts/website/bench-data/mion-bench.mjs).
  //   Their own image (mion-bench), so their own knobs; the RT_VALIDATION_BENCH_* set above drives
  ...
  {name: 'RT_DRIZZLE_DIALECT', scope: 'internal', task: '-', desc: 'Which lane the drizzle-e2e run is (pg | mysql | sqlite | d1 | durable; the last two are the Cloudflare storage drivers, not dialects); read by container/drizzle-e2e/shared/run-suite.mjs, passed in via -e by scripts/release/drizzle-e2e.mjs'},
  ...
  {name: 'RT_DRIZZLE_MINIFLARE_DIR', scope: 'internal', task: '-', desc: "Where miniflare persists the D1 database and the Durable Object SQL storage for one tree of the Cloudflare drizzle-e2e lanes. run-suite.mjs gives the control, builders and types trees one each, which is how those lanes get the isolation the server lanes get from a separate database"},
  {name: 'RT_DRIZZLE_HOME', scope: 'internal', task: '-', desc: "Overrides the drizzle-e2e install root (/drizzle-e2e) for container/drizzle-e2e/shared/runners/durable-worker.mjs, so the Durable Objects harness can be smoke-tested on the host against .cache/drizzle-suites/<tag>-translated with no container"},
  {name: 'RT_DRIZZLE_SHARED', scope: 'internal', task: '-', desc: 'Overrides the mounted shared-assets dir (/drizzle-src) for durable-worker.mjs; the host twin of RT_DRIZZLE_HOME'},
  ...
  {name: 'RT_VALIDATION_BENCH_RESULTS_DIR', scope: 'internal', task: '-', desc: 'In-container benchmark results dir (passed via -e)'},
  {name: 'RT_VALIDATION_BENCH_HOST_CPU', scope: 'internal', task: '-', desc: 'Host CPU model captured into env.json (passed via -e)'},
  {name: 'RT_VALIDATION_BENCH_REPO_ROOT', scope: 'internal', task: '-', desc: 'Serialization-bench repo root (passed via -e)'},
  {name: 'RT_VALIDATION_BENCH_VITE_ROOT', scope: 'internal', task: '-', desc: 'Serialization-bench vite root (passed via -e)'},
  {name: 'RT_VALIDATION_BENCH_PACKAGE_ROOT', scope: 'internal', task: '-', desc: 'Serialization-bench marker package root (passed via -e)'},
  {name: 'RT_VALIDATION_BENCH_RT_OUTDIR', scope: 'internal', task: '-', desc: 'Serialization-bench resolver output dir (passed via -e)'},
  {name: 'RT_VALIDATION_BENCH_BIN', scope: 'internal', task: '-', desc: 'Serialization-bench resolver binary path (passed via -e)'},
  {name: 'RT_VALIDATION_BENCH_PLUGIN_ENTRY', scope: 'internal', task: '-', desc: 'Serialization-bench vite plugin entry (passed via -e)'},
  {name: 'RT_EXTRACT_BIN', scope: 'internal', task: '-', desc: 'Serialization-bench fn-body extractor path (passed via -e)'},
  {name: 'RT_VALIDATION_BENCH_OUT_DIR', scope: 'internal', task: '-', desc: 'Serialization-bench output dir (passed via -e)'},
  {name: 'RT_VALIDATION_BENCH_SSR_NOEXTERNAL', scope: 'internal', task: '-', desc: 'Serialization-bench vite ssr.noExternal list (passed via -e)'},
  {name: 'RT_VALIDATION_BENCH_CACHE_DIR', scope: 'internal', task: '-', desc: "Serialization-bench resolver cache dir / false (passed via -e; forwarded to the binary's RT_CACHE_DIR)"},
];
