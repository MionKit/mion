...
// Slim table core: the runtime object a dialect's table factory returns (the column recorders as
// properties plus metadata under the rtTableKey symbol) and the traversal that materializes the real
// drizzle table on demand. A dialect's table function calls createRtTable with a buildTable closure
// that knows which drizzle factory to call; its toDrizzle calls materializeRtTable with the injected
// DrizzleContext.
...
/** A slim table's TYPE: a type-level DESCRIPTOR, not a mirror of the runtime object. The object
 *  createRtTable returns still carries the columns as properties and its recorded state under
 *  rtTableKey, because `extraConfig((t) => [index().on(t.name)])` and `references(() => teams.id)`
 *  read those at run time. Which dialect recorded it rides the separate RtTableBrand below.
...
/** Any slim table, of any dialect: the constraint the models and refineTableType take. */
...
/** The columns of a slim table, as a VALUE. Identity at run time: the object really does carry them
 *  as properties, the type just does not name them. How a cross-table reference is written:
