...
import type {SQLiteColumn, SQLiteTable, SQLiteTableWithColumns} from 'drizzle-orm/sqlite-core';
...
export type TableRefinements<T extends SQLiteTable> = {
...
type RefineColumn<Col, R> =
  Col extends SQLiteColumn<infer Config, infer RuntimeConfig, infer TypeConfig>
    ? SQLiteColumn<
...
export type RefinedTable<T extends SQLiteTable, R> =
  T extends SQLiteTable<infer Config>
    ? SQLiteTableWithColumns<{
        name: Config['name'];
        schema: Config['schema'];
        dialect: Config['dialect'];
        columns: Assume<RefineColumns<Config['columns'], R>, Record<string, SQLiteColumn>>;
      }>
    : never;
...
export type InferSelect<T extends SQLiteTable> = InferSelectModel<T>;
/** Insert payload of a (refined) table: generated columns removed, defaulted optional. */
export type InferInsert<T extends SQLiteTable> = InferInsertModel<T>;
/** Update payload of a (refined) table: any subset of the insert payload. */
export type InferUpdate<T extends SQLiteTable> = UpdateModel<InferInsertModel<T>>;
