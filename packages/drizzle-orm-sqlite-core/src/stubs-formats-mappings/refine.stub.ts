...
// param kept (sqlite integer() stamps the plain Integer format, so the
// refined min simply lands beside it).
type _nameMerges = Assert<Equal<ApiUser['name'], Str<{maxLength: 100; minLength: 10}>>>;
type _ageMerges = Assert<Equal<ApiUser['age'], RTNumber<{integer: true; min: 18}>>>;
// Untouched columns keep their exact former shape, nullability included
// (sqlite text without a length types as the wrapper's overload union).
type _bioUntouched = Assert<Equal<ApiUser['bio'], Str | Str<{maxLength: number}> | null>>;
