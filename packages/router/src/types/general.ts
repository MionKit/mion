...
import {AOTCaches, CoreRouterOptions, SerializerMode} from '@mionjs/core';
...
/** Internal router init state, shared via globalThis so all module instances see the same routes/options.
 *  `aotMode` is true when initRouter received `aotCaches` (and we're not currently emitting them) —
 *  reflection uses it to fail-loud on missing AOT entries instead of falling back to JIT-time reflection. */
...
/**
 * Options accepted by `initRouter` / `initMionRouter`.
 * Extends `RouterOptions` with `aotCaches` — a one-time bootstrap argument
 * that loads pre-compiled JIT functions and route metadata. The caches are
 * NOT stored in the router state (they're loaded into the global jit/pure/router
 * caches), so they don't appear in `getRouterOptions()` output.
 *
 * @example
 * import { aotCaches } from 'virtual:mion-aot/caches';
 * await initMionRouter(routes, { aotCaches });
 */
export type InitRouterOptions<Req = any, ContextData extends Record<string, any> = any> = Partial<
    RouterOptions<Req, ContextData>
> & {
    /**
     * Pre-compiled AOT caches generated by mionVitePlugin. When provided, the
     * router uses pre-compiled JIT functions and route metadata and will NOT
     * load `@mionjs/run-types` at runtime. Throws an `AOTCacheError` if any
     * route/middleFn is missing from the cache.
     */
    aotCaches?: AOTCaches;
};
