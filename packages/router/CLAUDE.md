...
`@ts-runtypes` resolves types at BUILD TIME from the TypeScript program and injects the compiled functions at the `route()` / `middleFn()` call site, so an erased import changes nothing. Guarded by [src/typeOnlyImports.spec.ts](src/typeOnlyImports.spec.ts).
