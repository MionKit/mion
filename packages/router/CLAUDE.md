...
`createMionRouter(opts)` ([src/router.ts](src/router.ts), types in [src/types/mionRouter.ts](src/types/mionRouter.ts)) is the ONLY public way to initialize the router and to declare routes / middleFns: the options are written once and ride by type (`O`) into every helper, so `ctx.shared` is typed from `contextDataFactory` and a build-time feature can compute marker slots from `O`. The runtime is still the module singleton: `mion.initRoutes(routes)` runs the private `initRouter` + `registerRoutes` in `src/router.ts`. The `serializer` option is the first such build-time feature: the helper signatures in [src/types/mionRouter.ts](src/types/mionRouter.ts) compute the marker families from the factory options and the route option literal (a `CompTimeArgs` parameter), and `src/router.ts` checks the compiled families against the option resolved at runtime. The helper bodies in [src/lib/handlers.ts](src/lib/handlers.ts) are internal (the internal client / error / serializer routes use them directly) and are NOT exported from `index.ts`. Never re-add bare `route()` exports or a second init entry point.
