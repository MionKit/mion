...
describe('the TypeScript floor notice', () => {
  ...
  it('warns once when the installed TypeScript is below the floor', () => {
    ...
    warnBelowTypeScriptFloor(dir, '@ts-runtypes/devtools', (line) => lines.push(String(line)));
    warnBelowTypeScriptFloor(dir, '@ts-runtypes/devtools', (line) => lines.push(String(line)));
  ...
  it.each([
    ...
    const lines: string[] = [];
    warnBelowTypeScriptFloor(projectWithTypeScript(version), '@ts-runtypes/devtools', (line) => lines.push(String(line)));
  ...
  it('never throws, whatever it finds', () => {
    ...
    expect(() => warnBelowTypeScriptFloor(broken, '@ts-runtypes/devtools', () => {})).not.toThrow();
    resetTypeScriptFloorCheckForTest();
    expect(() =>
      warnBelowTypeScriptFloor(path.join(os.tmpdir(), 'rt-ts-floor-missing'), '@ts-runtypes/devtools', () => {})
    ).not.toThrow();
  });
});
