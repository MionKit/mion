// Type-level format refinement: tighten a format-carrying type by merging extra params into the ones
// it already carries. The @mionjs/drizzle-orm-*-core packages build refineTableType on top of these.
...
/** Format family -> the params a refinement may add. A type carrying no family (or one missing here)
 *  refines to `never`, so ANY refinement on it is a compile error, never a silent bypass. */
...
/** The params a refinement may add to `T`, or `never` when `T` carries no refinable format. */
...
/** `T` rebuilt with `P` MERGED into its captured format params (`P` wins on a shared key). Base and
 *  family are preserved, so the value type can never change, only the params tighten. */
