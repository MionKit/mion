// The runtime half of the `jsonSchema` (jsc) cache family: everything the converters do at runtime to
// the document the compiled entry returns (the target check, the portable strip, the encoder-strategy
// closedness stamp). ONE document serves both converter sides — the standard keywords describe the JSON
// wire, the dialect keywords annotate what that JSON becomes in JS — so `input()` and `output()` return
// the SAME document: RunTypes validates without transforming, the non-transforming shape the Standard
// Schema spec permits. `encoderStrategy` follows the same rule, an explicit caller declaration of the
// wire the paired JSON encoder produces.
...
/** The one target this library emits; other dialects throw. **/
export const JSON_SCHEMA_TARGET = 'draft-2020-12';

/** The JS-extension rows the Go renderer can emit beside the standard vocabulary; `{portable: true}`
 *  strips exactly these. A keyword added on the Go side without a row here survives the strip and fails
 *  the doc-suite's portable assertions. **/
...
// Keys whose values are JSON DATA, not subschemas: the closedness walk must not descend into them, as a
// literal value could coincidentally look like an object schema. `tsMeta` is NOT here, its halves are real schemas.
const VALUE_POSITION_KEYS = new Set(['const', 'enum', 'default', 'examples', 'rtFormatParams']);

/** Deep-stamps `additionalProperties: false` onto every KEYED object node. Records (whose
 *  `additionalProperties` carries the index schema) and the bare `object` keyword are left alone:
 *  closing a node with no `properties` would read as "no keys at all". **/
...
/** Deep-strips the dialect keywords, returning plain 2020-12. A `tsMeta` node collapses to its stripped
 *  base (the metadata halves are annotation-only), and a node left empty reads as `{}`, any value. **/
