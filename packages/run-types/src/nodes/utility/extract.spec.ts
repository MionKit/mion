...
describe('Extract typescript utility type, extract atomic elements from an union', () => {
    ...
    it('json encode/decode', () => {
        ...
        const encodeExtract = rtExtract.createJitFunction(JitFunctions.prepareForJson);
        const decode = rt.createJitFunction(JitFunctions.restoreFromJson);
        const decodeExtract = rtExtract.createJitFunction(JitFunctions.restoreFromJson);
    ...
    // Test moved to packages/run-types/src/jitCompilers/json/stringifyJson.spec.ts (lines 242-250)
...
describe('Extract typescript utility type, extract items from objects union', () => {
    ...
    // Test moved to packages/run-types/src/jitCompilers/json/stringifyJson.spec.ts (lines 263-271)
