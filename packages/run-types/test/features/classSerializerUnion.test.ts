...
describe('classSerializer union / same-shape classes distinguished by identity', () => {
  ...
  it('reflect (JSON) — same-shape members stay distinct through the union index', () => {
    registerPair();
    const sample: Pair = new Vec(0, 0);
    const encode = createJsonEncoderFn(sample);
    const decode = createJsonDecoderFn(sample);

    expect(decode(encode(new Vec(3, 4)) as string)).toBeInstanceOf(Vec);
    expect(decode(encode(new Loc(3, 4)) as string)).toBeInstanceOf(Loc);
  });
});
...
describe('classSerializer union / class + plain object', () => {
  ...
  it('reflect (JSON) — class instance vs plain object round-trip', () => {
    registerClassSerializer(Coin, {deserialize: (d) => new Coin(d.cents)});
    const sample: Money = new Coin(0);
    const encode = createJsonEncoderFn(sample);
    const decode = createJsonDecoderFn(sample);

    expect(decode(encode(new Coin(99)) as string)).toBeInstanceOf(Coin);
    expect(decode(encode({note: 'hi'}) as string)).toEqual({note: 'hi'});
  });
});
...
describe('classSerializer union / unregistered members fall back to plain objects', () => {
  ...
  it('reflect (JSON) — unregistered class union decodes to plain objects (no throw)', () => {
    const sample: Shape = new Circle(0);
    const encode = createJsonEncoderFn(sample);
    const decode = createJsonDecoderFn(sample);
    ...
    let decoded: unknown;
    expect(() => {
      decoded = decode(encode(new Square(6)) as string);
    }).not.toThrow();
...
describe('classSerializer union / a base class and its subclass', () => {
  ...
  it('reflect (JSON) — the subclass reconstructs as itself whichever member is declared first', () => {
    registerErrs();
    const sample: BaseFirst = new BaseErr('x');
    const encode = createJsonEncoderFn(sample);
    const decode = createJsonDecoderFn(sample);
    const fatal = decode(encode(new FatalErr('gate')) as string) as FatalErr;
    expect(fatal).toBeInstanceOf(FatalErr);
    expect(fatal.isFatal).toBe(true);
