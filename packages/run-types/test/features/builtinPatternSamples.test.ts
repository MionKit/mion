...
// The built-in string patterns used to check their own mockSamples at module load,
// through a side-effect import in formats/index.ts. That import shipped the whole
// pattern table to every browser bundle, so it is gone and the check lives here.
...
describe('built-in string patterns', () => {
  ...
  it.each(entries)('%s accepts every mockSample it declares', (name, pattern) => {
    // g/y make .test advance lastIndex, so the tester drops them, exactly as
    // registerFormatPattern does.
