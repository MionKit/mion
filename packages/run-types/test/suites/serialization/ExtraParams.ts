...
export const EXTRA_PARAMS = {
  ...
  extras_inside_atomic_union_member: {
    ...
    description:
      'Extra `extra: "hello"` sits on the object inside the array member of `{declared: string}[] | number`: the unsafe path round-trips it intact, clone and compact strip it before serialise, and the clone decoder drops it when a caller sends it.',
  ...
  extras_inside_tuple_slot: {
    ...
    description:
      'Extra `extra: "hello"` sits on the object in the first slot of `[{declared: string}, number]`: the unsafe path round-trips it intact, clone and compact strip it before serialise, and the clone decoder drops it when a caller sends it.',
    mutateEncoder: () => createJsonEncoderFn<[{declared: string}, number]>(undefined, {strategy: 'mutate'}),
    cloneEncoder: () => createJsonEncoderFn<[{declared: string}, number]>(undefined, {strategy: 'clone'}),
    compactEncoder: () => createJsonEncoderFn<[{declared: string}, number]>(undefined, {strategy: 'compact'}),
    cloneDecoder: () => createJsonDecoderFn<[{declared: string}, number]>(),
