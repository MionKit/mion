...
export const TEMPLATE_LITERALS = {
  url_string: {
    ...
    description:
      'Root template-literal type `` `api/users/${number}` `` round-trips identically through JSON as a plain string, with samples covering integer, negative, fractional, and max-safe-integer interpolations.',
    serializeNotes:
    ...
    cloneDecoder: () => createJsonDecoderFn<`api/users/${number}`>(),
    mutateDecoder: () => createJsonDecoderFn<`api/users/${number}`>(undefined, {strategy: 'mutate'}),
  ...
  url_in_object: {
    ...
    description:
      'Object with a template-literal-typed `url` property plus a plain `method: string` round-trips identically through JSON as plain strings.',
    ...
    cloneDecoder: () => createJsonDecoderFn<{url: `api/user/${number}`; method: string}>(),
    mutateDecoder: () => createJsonDecoderFn<{url: `api/user/${number}`; method: string}>(undefined, {strategy: 'mutate'}),
  ...
  url_index_key: {
    ...
    description:
      'Record whose index-signature key is a template literal `` `api/${string}` `` with `number` values round-trips as a plain key/value object through JSON, including the empty-object case.',
    serializeNotes:
    ...
    cloneDecoder: () => createJsonDecoderFn<{[key: `api/${string}`]: number}>(),
    mutateDecoder: () => createJsonDecoderFn<{[key: `api/${string}`]: number}>(undefined, {strategy: 'mutate'}),
  ...
  url_index_key_with_named: {
    ...
    description:
      'Object combining a template-literal-keyed index signature (`` `api/${string}` `` → `string | number`) with a sibling named `meta: string` resolves to an intersection of the keyed record and an object carrying the named prop, round-tripping as a plain object through JSON.',
    mutateEncoder: () =>
    ...
    cloneDecoder: () => createJsonDecoderFn<{meta: string; [key: `api/${string}`]: string | number}>(),
    mutateDecoder: () =>
      createJsonDecoderFn<{meta: string; [key: `api/${string}`]: string | number}>(undefined, {strategy: 'mutate'}),
    compactDecoder: () =>
