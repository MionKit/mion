...
export const TEMPLATE_LITERALS = {
  url_string: {
    ...
    clone: () => createRemoveUnknownKeysFn<`api/users/${number}`>(),
    getTestData: () => ({
      values: [
        'api/users/0',
        'api/users/1',
  ...
  url_in_object: {
    ...
    clone: () => createRemoveUnknownKeysFn<{url: `api/user/${number}`; method: string}>(),
    getTestData: () => ({
      values: [
  ...
  url_index_key: {
    ...
    clone: () => createRemoveUnknownKeysFn<{[key: `api/${string}`]: number}>(),
  ...
  url_index_key_with_named: {
    ...
    clone: () => createRemoveUnknownKeysFn<{meta: string; [key: `api/${string}`]: string | number}>(),
    getTestData: () => ({
