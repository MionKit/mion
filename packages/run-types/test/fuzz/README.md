...
Holds a small set of concrete schemas and floods each with **five** value
...
- `unknownKeyPositions.ts` — the unknown-key position walker: every position
  that has its own arm in the unknown-keys emitters (root object, union member
  object AND named class, nested property, array item, tuple slot, Map key and
  value, Set member, index signature), each labelled `flagged` (every family
  owes a report) or `carveOut` (an index signature declares every key, so every
  family owes silence). Deliberately a SECOND walker beside
  `cloning/extrasValue.ts`, which stays conservative for the clone oracle.
...
- `fuzz.integration.test.ts` — 100 iterations × the concrete targets through the
  ...
  `invalidValue` / `unknownKeyPositions` against a reference structural
...
(`uke`), `cloneExactShape` (`ces`, the public strip that replaced the removed
