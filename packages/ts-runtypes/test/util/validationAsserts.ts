...
/** JSON-Schema form: createValidateFn(runTypeFromJsonSchema({…})). Proves the schema-literal
...
export function assertValidateJsonSchema(c: AssertableCase): void {
  ...
  const {valid, invalid} = (c.getJsonSchemaSamples ?? c.getSamples)();
...
/** JSON-Schema form: createGetValidationErrorsFn(runTypeFromJsonSchema({…})). Same
