// id-integrity / jsonSchema — for EVERY validation + format-validation +
// serialization + format-serialization case whose jsonSchema thunks are filled,
// assert the jsonSchema-authored form (`createValidateFn(jsonSchema({…}))` /
// `createJsonEncoderFn(jsonSchema({…}))`) resolves the SAME runtype as the
...
// this driver goes live automatically as the jsonSchema column drains
...
describe('id-integrity / jsonSchema validators — schema-literal ↔ type-first resolve one cached factory', () => {
  registerValidators('validation', VALIDATION_SUITE);
  registerValidators('format-validation', FORMAT_VALIDATION_SUITE);
});

describe('id-integrity / jsonSchema serializers — schema-literal encoder output equals type-first', () => {
