...
// this test keeps it empty: a NEW case added without deciding its jsonSchema
...
// type without the jsonSchema column.
...
describe('json-schema column completion — zero "(not implemented)" omissions', () => {
  it('every validation + format-validation case decides both jsonSchema fields', () => {
    ...
    expect([...validation.missing, ...formatValidation.missing], 'omitted (pending) jsonSchema fields').toEqual([]);
    expect(
  ...
  it('every serialization + format-serialization case decides all four jsonSchema thunks', () => {
    ...
    expect([...serialization.missing, ...formatSerialization.missing], 'omitted (pending) jsonSchema thunks').toEqual([]);
    expect(
