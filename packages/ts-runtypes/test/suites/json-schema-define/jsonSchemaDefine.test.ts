...
// runTypeFromJsonSchema), and the CLAUDE.md marker-rule paired `getRunTypeId` shapes with
...
} from '@ts-runtypes/core/json-schema';
...
describe('json-schema define — marker coverage rule (paired getRunTypeId shapes)', () => {
  it('static <T>, reflection (value) and builder forms resolve ONE hash', () => {
    ...
    expect(getRunTypeId(runTypeFromJsonSchema(USER_SCHEMA))).toBe(idStatic);
...
describe('json-schema define — idempotency', () => {
  it('the same module-const schema at two call sites resolves ONE cached factory', () => {
    expect(createValidateFn(runTypeFromJsonSchema(USER_SCHEMA))).toBe(createValidateFn(runTypeFromJsonSchema(USER_SCHEMA)));
  });
  ...
  it('a key-order permutation of the same schema resolves the same id', () => {
    const orderedId = getRunTypeId(runTypeFromJsonSchema({type: 'string', minLength: 2, maxLength: 50}));
    const permutedId = getRunTypeId(runTypeFromJsonSchema({maxLength: 50, minLength: 2, type: 'string'}));
    expect(permutedId).toBe(orderedId);
  });
});

describe('json-schema define — three-way convergence (type-first ↔ value-first ↔ runTypeFromJsonSchema)', () => {
...
describe('json-schema define — boolean root schemas (2020-12)', () => {
  it('runTypeFromJsonSchema(true) recovers unknown and converges with the type-first form', () => {
    expect(createValidateFn(runTypeFromJsonSchema(true))).toBe(createValidateFn<unknown>());
  });

  it('runTypeFromJsonSchema(false) recovers never — the reject-all validator, converging with type-first', () => {
    const isNever = createValidateFn(runTypeFromJsonSchema(false));
...
describe('json-schema define — $defs and $ref recursion (M6)', () => {
  it("$ref: '#' re-enters the root: circular array converges with the type-first twin", () => {
    ...
    const isCircular = createValidateFn(runTypeFromJsonSchema({type: 'array', items: {$ref: '#'}}));
...
describe('json-schema define — sample-less pattern mocking (build-generated samples)', () => {
  it('mocking a schema-pattern type draws from the build-generated sample pool', () => {
    ...
    const isSlug = createValidateFn(runTypeFromJsonSchema({type: 'string', pattern: '^[a-z-]+$'}));
    const mockSlug = createMockDataFn(runTypeFromJsonSchema({type: 'string', pattern: '^[a-z-]+$'}));
    for (let round = 0; round < 8; round++) {
...
describe('json-schema define — /schema utility builders compose over runTypeFromJsonSchema results (M5)', () => {
  // The utility builders take any RunType<T>, so a runTypeFromJsonSchema(...) result flows
  ...
  it('partial / required / pick / omit / readonlyType compose over a schema-authored object', () => {
    const base = () =>
      runTypeFromJsonSchema({type: 'object', properties: {a: {type: 'string'}, b: {type: 'number'}}, required: ['a', 'b']});
  ...
  it('nonNullable / exclude / extract compose over schema-authored unions', () => {
    expect(createValidateFn(RT.nonNullable(runTypeFromJsonSchema({anyOf: [{type: 'string'}, {type: 'null'}]})))).toBe(
      createValidateFn<string>()
    );
    expect(createValidateFn(RT.exclude(runTypeFromJsonSchema({enum: ['a', 'b', 'c']}), RT.literal('c')))).toBe(
      createValidateFn<'a' | 'b'>()
    );
    expect(
      createValidateFn(RT.extract(runTypeFromJsonSchema({enum: ['a', 'b', 'c']}), runTypeFromJsonSchema({enum: ['a', 'b']})))
    ).toBe(createValidateFn<'a' | 'b'>());
  });
});
...
describe('json-schema define — ignored annotations', () => {
  it('$schema/title/description/examples/default do not change the resolved id', () => {
    const bare = createValidateFn(runTypeFromJsonSchema({type: 'string', minLength: 3}));
  ...
  it('readOnly is an annotation at property positions — the mutable twin owns the entry (static shape)', () => {
    // The property-position readonly lift was removed on purpose: the keyword
    // validates nothing, and an annotation must never move the structural id.
    // The schema lands on the MUTABLE hand-written twin's cache entry.
    ...
    expect(annotated).toBe(getRunTypeId<{id: string; name: string}>());
  });

  it('readOnly is an annotation at property positions (reflected value shape)', () => {
    const value: {id: string; name: string} = {id: 'u_1', name: 'ada'};
...
describe("json-schema define — numbers follow OUR semantics, not the dialect's", () => {
  // JSON Schema says nothing about NaN / Infinity (JSON has no way to write
  // them), so a translator gets to choose. The schema form must NOT invent its
  // own answer: it recovers plain `number`, which means it inherits whatever
  // `numberMode` is in force — `isFinite` by default. That is the same validator
  // the type-first form gets, which is the whole point of the third door.
  //
  // The consequence is a REAL, intentional divergence from ajv, whose
  // `{type: 'number'}` accepts NaN and Infinity. It is reported, not hidden: the
  // benchmark alignment audit compares every competitor against the shared
  // samples (which encode our behaviour), so it lands on the correctness page.
  // Aligning to ajv is a caller's choice via `numberMode: 'typeof'`, never a
  // property of the authoring form.
  it('a schema-authored number rejects NaN and Infinity, exactly as type-first does', () => {
    const isNumber = createValidateFn(runTypeFromJsonSchema({type: 'number'}));
    expect(isNumber).toBe(createValidateFn<number>());
  ...
  it('holds through a union arm and a record value', () => {
    const isUnion = createValidateFn(runTypeFromJsonSchema({anyOf: [{type: 'string'}, {type: 'number'}, {type: 'null'}]}));
    expect(isUnion('a')).toBe(true);
    expect(isUnion(42)).toBe(true);
    expect(isUnion(null)).toBe(true);
    expect(isUnion(NaN)).toBe(false);
    expect(isUnion(Infinity)).toBe(false);

    const isRecord = createValidateFn(runTypeFromJsonSchema({type: 'object', additionalProperties: {type: 'number'}}));
    expect(isRecord({a: 1})).toBe(true);
    expect(isRecord({a: NaN})).toBe(false);
    expect(isRecord({a: Infinity})).toBe(false);
  });

  it('an integer schema rejects them too, and here ajv agrees (NaN is not an integer)', () => {
    const isAge = createValidateFn(runTypeFromJsonSchema({type: 'integer', minimum: 0, maximum: 130}));
    expect(isAge(36)).toBe(true);
    expect(isAge(NaN)).toBe(false);
    expect(isAge(Infinity)).toBe(false);
    expect(isAge(36.5)).toBe(false);
  });
});
