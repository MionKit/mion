...
// No-plugin fallback: a document cannot be improvised at runtime (there is no
// runtime reflection), so resolving without a compiled entry throws with the
// standard guidance instead of returning a silently-wrong `{}`.
