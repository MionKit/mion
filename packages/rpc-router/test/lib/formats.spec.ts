...
import {getSerializableMethod, serializeMethodDeps} from '../../src/lib/remoteMethods.ts';
...
describe('Dispatch routes with format types', () => {
  ...
  describe('format validation should', () => {
    it('pass validation with valid format data', async () => {
      ...
      const response = await dispatchRoute('/createUser', request.body, request.headers, headersFromRecord({}), request, {});
