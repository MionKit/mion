...
// each approach and nothing else.
//
// The two alternative lanes get budgets on the same one-way-downward terms as
// the drizzle lane (see modelPipeline.compile.test.ts for the full rule). The
// drizzle lane is re-measured here rather than read from its budgets, so the
// comparison is measurement against measurement.
//
// What this suite does NOT do is pick a winner. The drizzle lane derives the
...
/** The drizzle lane wrapped as a Lane so all three measure through one path. **/
const DRIZZLE_LANE: Lane = {
  name: 'drizzle',
...
const LANES: Lane[] = [DRIZZLE_LANE, ...ALTERNATIVE_LANES];
...
function measureLane(lane: Lane): number[] {
  ...
  for (let i = 0; i < lane.steps.length; i++) {
    const result = lane.measure(lane === DRIZZLE_LANE ? snippetUpTo(i) : laneSnippetUpTo(lane, i));
    expect(
      result.errors,
...
describe('model declaration approaches — cost comparison', () => {
  ...
  // Steps 5 and 6 are the same text in every lane, so their deltas should stay
  ...
  // readonly params, the drizzle lane's aliases), and step 6 spreads 21% today
  // (2335 / 2601 / 2817). The threshold sits above that to catch drift, not to
  // pin the current gap.
  it('the shared route and client steps cost about the same in every lane', () => {
    for (const stepIndex of [4, 5]) {
