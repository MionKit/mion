...
`,
    ...
    // route api 557 -> 564: per-route serializer strategies, the literal check on the factory call (see modelPipelineHarness).
    ...withTailBudgets([393, 254, 564, 2651]),
  ],
...
`,
    ...
    // directly, no Promise unwrap), so the cost moved from the route api step to the client step.
    // route api 553 -> 560: per-route serializer strategies, the literal check on the factory call (see modelPipelineHarness).
    ...withTailBudgets([384, 262, 560, 2867]),
  ],
