...
register('batch transport through a real vite build', () => {
  ...
  it('generates the table and the mapper module under the gen dir, relative imports only', async () => {
    ...
    expect(table).toMatch(/import \{__rt_pf[A-Za-z0-9_$]+\} from '\.\/pf\/[^']+\.js';/);
    ...
    // one generated pure-fn module, under the path the mapper's id names
    const mapperModule = /from '\.\/(pf\/[^']+\.js)'/.exec(table)?.[1];
    expect(mapperModule, table).toBeDefined();
    expect(readFileSync(path.join(rpc, mapperModule!), 'utf8')).toContain(MAPPER_BODY);
  });
