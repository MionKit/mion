...
function getObjectTypeProperties(node) {
	if (node.type === AST_NODE_TYPES.TSTypeLiteral) {
		const props = [];
		for (const member of node.members) if (member.type === AST_NODE_TYPES.TSPropertySignature && member.key.type === AST_NODE_TYPES.Identifier) props.push({
...
function getObjectLiteralPropertyNames(node) {
	const names = [];
	for (const prop of node.properties) if (prop.type === AST_NODE_TYPES.Property && prop.key.type === AST_NODE_TYPES.Identifier) names.push(prop.key.name);
	return names;
}
...
function resolveTypeReference(node, context) {
	...
	if (node.typeName.type !== AST_NODE_TYPES.Identifier) return null;
	const typeName = node.typeName.name;
	const program = context.sourceCode.ast;
	for (const statement of program.body) if (statement.type === AST_NODE_TYPES.TSTypeAliasDeclaration) {
		if (statement.id.name === typeName && statement.typeAnnotation.type === AST_NODE_TYPES.TSUnionType) return statement.typeAnnotation;
	}
	return null;
}
function getReturnTypeAnnotation(func, context) {
	...
	if (returnType.type === AST_NODE_TYPES.TSUnionType) return returnType;
	if (returnType.type === AST_NODE_TYPES.TSTypeReference) return resolveTypeReference(returnType, context);
	return null;
}
function findReturnStatements(func) {
	...
	if (func.body.type === AST_NODE_TYPES.BlockStatement) findReturnsInBlock(func.body, returns);
	return returns;
}
function findReturnsInBlock(block, returns) {
	for (const statement of block.body) if (statement.type === AST_NODE_TYPES.ReturnStatement && statement.argument) returns.push(statement.argument);
	else if (statement.type === AST_NODE_TYPES.IfStatement) {
		if (statement.consequent.type === AST_NODE_TYPES.BlockStatement) findReturnsInBlock(statement.consequent, returns);
		else if (statement.consequent.type === AST_NODE_TYPES.ReturnStatement && statement.consequent.argument) returns.push(statement.consequent.argument);
		if (statement.alternate?.type === AST_NODE_TYPES.BlockStatement) findReturnsInBlock(statement.alternate, returns);
		else if (statement.alternate?.type === AST_NODE_TYPES.ReturnStatement && statement.alternate.argument) returns.push(statement.alternate.argument);
	}
}
...
function isRouterFunction(node, context) {
	let current = node;
	while (current) {
		if (current.type === AST_NODE_TYPES.CallExpression) return isRouterFunctionCall(current, context);
...
function isImportedFromMionRouter(name, context) {
	...
	for (const statement of program.body) if (statement.type === AST_NODE_TYPES.ImportDeclaration) {
		...
		if (source === "@mionjs/router" || source === "@mionjs/router/") {
			for (const specifier of statement.specifiers) if (specifier.type === AST_NODE_TYPES.ImportSpecifier && specifier.imported.type === AST_NODE_TYPES.Identifier && specifier.imported.name === name) return true;
		}
	}
	return false;
}
