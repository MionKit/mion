		...
		return {
			Program(node) {
				...
				for (const statement of node.body) {
					...
					for (const specifier of statement.specifiers) if (specifier.type === AST_NODE_TYPES.ImportSpecifier && specifier.imported.type === AST_NODE_TYPES.Identifier && specifier.imported.name === "serverMapFrom") pureFnNames.set(specifier.local.name, specifier.imported.name);
				}
			},
			CallExpression(node) {
				...
				if (!mapperOrName || mapperOrName.type === AST_NODE_TYPES.ArrowFunctionExpression || mapperOrName.type === AST_NODE_TYPES.FunctionExpression) context.report({
					node,
					messageId: "missingMapFromName"
				});
				else if (mapperOrName.type !== AST_NODE_TYPES.Literal || typeof mapperOrName.value !== "string") context.report({
					node: mapperOrName,
