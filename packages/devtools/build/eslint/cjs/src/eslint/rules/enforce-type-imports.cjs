...
//#region src/eslint/rules/enforce-type-imports.ts
/** Enforce type-only imports from backend code to prevent bundling server code into the frontend. */
var rule = {
	meta: {
		type: "suggestion",
		fixable: "code",
		docs: { description: "Enforce type-only imports from backend code paths to prevent bundling server code into the frontend." },
		messages: {
	...
	create(context) {
		...
		function hasAllValueSpecifiers(specifiers) {
			return specifiers.every((s) => {
				if (s.type === _typescript_eslint_utils.AST_NODE_TYPES.ImportSpecifier) return s.importKind !== "type";
				return true;
			});
		}
		function fixImport(fixer, node) {
			...
			const fixes = [];
			for (const specifier of node.specifiers) if (specifier.type === _typescript_eslint_utils.AST_NODE_TYPES.ImportSpecifier && specifier.importKind !== "type") fixes.push(fixer.insertTextBefore(specifier, "type "));
			return fixes;
		}
		function fixExport(fixer, node) {
			const sourceCode = context.sourceCode;
			if (node.specifiers.every((s) => s.exportKind !== "type")) {
				const exportToken = sourceCode.getFirstToken(node);
				return fixer.insertTextAfter(exportToken, " type");
			}
			const fixes = [];
			for (const specifier of node.specifiers) if (specifier.exportKind !== "type") fixes.push(fixer.insertTextBefore(specifier, "type "));
			return fixes;
		}
		return {
			...
			ExportNamedDeclaration(node) {
				...
				if (!node.specifiers.some((s) => s.exportKind !== "type")) return;
				context.report({
					node,
