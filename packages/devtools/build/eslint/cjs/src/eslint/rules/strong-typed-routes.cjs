...
function buildImportCache(program) {
	...
	for (const statement of program.body) if (statement.type === _typescript_eslint_utils.AST_NODE_TYPES.ImportDeclaration) {
		...
		if (source === "@mionjs/router" || source === "@mionjs/router/") {
			for (const specifier of statement.specifiers) if (specifier.type === _typescript_eslint_utils.AST_NODE_TYPES.ImportSpecifier && specifier.imported.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier) {
...
function getRouterFunctionName(node, importCache) {
	if (node.callee.type !== _typescript_eslint_utils.AST_NODE_TYPES.Identifier) return null;
...
function buildFunctionCache(program) {
	...
	for (const statement of program.body) {
		if (statement.type === _typescript_eslint_utils.AST_NODE_TYPES.FunctionDeclaration && statement.id?.name) cache.set(statement.id.name, statement);
		if (statement.type === _typescript_eslint_utils.AST_NODE_TYPES.VariableDeclaration) {
			for (const declarator of statement.declarations) if (declarator.id.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier) {
				if (declarator.init?.type === _typescript_eslint_utils.AST_NODE_TYPES.ArrowFunctionExpression || declarator.init?.type === _typescript_eslint_utils.AST_NODE_TYPES.FunctionExpression) cache.set(declarator.id.name, declarator.init);
			}
		}
	}
	return cache;
...
function getHandlerFunction(node, functionCache) {
	...
	if (handlerArg.type === _typescript_eslint_utils.AST_NODE_TYPES.ArrowFunctionExpression || handlerArg.type === _typescript_eslint_utils.AST_NODE_TYPES.FunctionExpression) return handlerArg;
	if (handlerArg.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier) return functionCache.get(handlerArg.name) ?? null;
	return null;
}
/**
...
function isPrimitiveLiteral(node) {
	if (node.type === _typescript_eslint_utils.AST_NODE_TYPES.Literal) {
	...
	if (node.type === _typescript_eslint_utils.AST_NODE_TYPES.UnaryExpression && node.operator === "-" && node.argument.type === _typescript_eslint_utils.AST_NODE_TYPES.Literal) return typeof node.argument.value === "number";
	return false;
}
/**
...
function validateParameterTypes(func) {
	...
	for (let i = 1; i < func.params.length; i++) {
		...
		if (param.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier) {
			if (!param.typeAnnotation) {
				missingTypeParams.push(param.name);
				missingParamNodes.push(param);
			}
		} else if (param.type === _typescript_eslint_utils.AST_NODE_TYPES.RestElement) {
			if (param.argument.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier && !param.typeAnnotation) {
				missingTypeParams.push(`...${param.argument.name}`);
				missingParamNodes.push(param);
			}
		} else if (param.type === _typescript_eslint_utils.AST_NODE_TYPES.ArrayPattern || param.type === _typescript_eslint_utils.AST_NODE_TYPES.ObjectPattern) {
			if (!param.typeAnnotation) {
				missingTypeParams.push(`parameter ${i + 1}`);
				missingParamNodes.push(param);
			}
		} else if (param.type === _typescript_eslint_utils.AST_NODE_TYPES.AssignmentPattern) {
			const hasTypeAnnotationOnPattern = param.typeAnnotation !== void 0;
			const hasTypeAnnotationOnLeft = param.left.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier && param.left.typeAnnotation !== void 0;
			...
			if (!hasTypeAnnotation && !hasPrimitiveDefault) {
				const paramName = param.left.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier ? param.left.name : `parameter ${i + 1}`;
...
function getParameterName(param) {
	if (param.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier) return param.name;
	else if (param.type === _typescript_eslint_utils.AST_NODE_TYPES.RestElement && param.argument.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier) return `...${param.argument.name}`;
	else if (param.type === _typescript_eslint_utils.AST_NODE_TYPES.ArrayPattern) return "[...]";
	else if (param.type === _typescript_eslint_utils.AST_NODE_TYPES.ObjectPattern) return "{...}";
	else if (param.type === _typescript_eslint_utils.AST_NODE_TYPES.AssignmentPattern) {
		if (param.left.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier) return param.left.name;
		return "param";
	}
	return "param";
}
...
function getReturnTypeReportNode(func) {
	if ((func.type === _typescript_eslint_utils.AST_NODE_TYPES.FunctionDeclaration || func.type === _typescript_eslint_utils.AST_NODE_TYPES.FunctionExpression) && func.id) return func.id;
	return func;
}
/**
...
function getHandlerTypeFromAnnotation(typeAnnotation, importCache) {
	if (typeAnnotation.typeAnnotation.type === _typescript_eslint_utils.AST_NODE_TYPES.TSTypeReference) {
...
function getHandlerTypeFromSatisfies(satisfiesExpression, importCache) {
	if (satisfiesExpression.typeAnnotation.type === _typescript_eslint_utils.AST_NODE_TYPES.TSTypeReference) {
	...
	create(context) {
		...
		return {
			...
			VariableDeclarator(node) {
				...
				if (node.id.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier) {
					let handlerType = null;
					if (node.id.typeAnnotation) handlerType = getHandlerTypeFromAnnotation(node.id.typeAnnotation, importCache);
					if (!handlerType && node.parent?.type === _typescript_eslint_utils.AST_NODE_TYPES.VariableDeclaration) handlerType = getHandlerTypeFromJSDoc(node.parent, context);
					if (handlerType && (node.init?.type === _typescript_eslint_utils.AST_NODE_TYPES.ArrowFunctionExpression || node.init?.type === _typescript_eslint_utils.AST_NODE_TYPES.FunctionExpression)) checkHandlerFunction(node.init, handlerType, context);
				}
			},
			TSSatisfiesExpression(node) {
				if (!importCache) return;
				const handlerType = getHandlerTypeFromSatisfies(node, importCache);
				if (handlerType && (node.expression.type === _typescript_eslint_utils.AST_NODE_TYPES.ArrowFunctionExpression || node.expression.type === _typescript_eslint_utils.AST_NODE_TYPES.FunctionExpression)) checkHandlerFunction(node.expression, handlerType, context);
			},
...
//# sourceMappingURL=strong-typed-routes.cjs.map