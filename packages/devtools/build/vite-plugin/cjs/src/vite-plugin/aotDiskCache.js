...
function getDevtoolsVersion() {
  ...
  try {
    const pkgPath = path.resolve(path.dirname(new URL(typeof document === "undefined" ? require("url").pathToFileURL(__filename).href : _documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === "SCRIPT" && _documentCurrentScript.src || new URL("src/vite-plugin/aotDiskCache.js", document.baseURI).href).pathname), "../../package.json");
...
function computeSourceHash(options) {
  const serverDir = path.dirname(path.resolve(options.startServerScript));
...
async function getOrGenerateAOTCaches(options, cacheDir) {
  ...
  const data = await src_vitePlugin_aotCacheGenerator.generateAOTCaches(options);
  if (cachingEnabled) {
