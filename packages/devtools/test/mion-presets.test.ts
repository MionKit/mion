...
describe('the api pointer and bundleApi — the client build bundles what it calls', () => {
  it('maps api.tsConfig and bundleApi onto the resolver options, as written', () => {
    const mapped = toRunTypesOptions({}, undefined, {api: {tsConfig: '../server/tsconfig.json'}, bundleApi: 'bundled'});
    expect(mapped.apiTsconfig).toBe('../server/tsconfig.json');
    expect(mapped.bundleApi).toBe('bundled');
    expect(toRunTypesOptions({}, undefined, {bundleApi: 'mixed'}).bundleApi).toBe('mixed');
    // off by default: an unset pair forwards nothing, so the fetched lane stays the default
    expect(toRunTypesOptions({}).apiTsconfig).toBeUndefined();
    expect(toRunTypesOptions({}).bundleApi).toBeUndefined();
  });

  it('rejects an empty pointer and an unknown mode', () => {
    expect(() => toRunTypesOptions({}, undefined, {api: {tsConfig: ''}})).toThrow(/api\.tsConfig/);
    expect(() => toRunTypesOptions({}, undefined, {bundleApi: 'all' as never})).toThrow(/bundleApi/);
  });

  it('is reached through BOTH presets', async () => {
    await expect(withMion({}, {api: {tsConfig: ''}, cwd: '/tmp'})).rejects.toThrow(/api\.tsConfig/);
    await expect(withMion({}, {bundleApi: 'all' as never, cwd: '/tmp'})).rejects.toThrow(/bundleApi/);
    expect(() => mionVitePlugin({api: {tsConfig: ''}})).toThrow(/api\.tsConfig/);
    expect(() => mionVitePlugin({bundleApi: 'all' as never})).toThrow(/bundleApi/);
  });
});
