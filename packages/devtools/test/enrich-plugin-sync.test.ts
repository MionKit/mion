...
function makePlugin(project: Project, options: PluginOptionsLite) {
  ...
  const plugin = (Array.isArray(produced) ? produced.find((entry: any) => entry?.name === '@mionjs/devtools') : produced) as any;
...
describeIfBinary('@mionjs/devtools / plugin-driven enrichment sync', () => {
