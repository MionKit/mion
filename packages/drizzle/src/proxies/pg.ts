...
// The @mionjs/drizzle/pg proxy: the full drizzle-orm/pg-core surface passes
// through unchanged, and the column builders below are shadowed by wrappers
// that forward the call verbatim while stamping the builder's data type with
// the matching runtype format (type-only, via drizzle's own $Type channel),
// capturing the caller's literal config params in OUR generics. Coverage is
// gated by packages/drizzle/manifests/<dialect>.manifest.json
