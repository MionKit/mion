...
// next/ mysql columns through the REAL resolver, each random spec as a hand-written type and as builders in one fixture.
...
import {int, mysqlTable} from '../../next/index.ts';

const nextParent = mysqlTable(FUZZ_PARENT_NAME, {id: int('id', {primaryKey: true})});
...
const FIXTURE = 'packages/drizzle-orm-mysql-core/__drizzleNextTypeFuzz__.ts';
...
function renderFixture(rng: () => number, iteration: number): Rendered {
  ...
  const source =
    ...
    `const NXV = {...DBV, ...NXM};\n` +
    `const fzParent = NXV.mysqlTable('${FUZZ_PARENT_NAME}', {id: NXV.int('id', {primaryKey: true})});\n` +
...
describe('next mysql columns fuzz: authored source through the real resolver', () => {
  register(
    ...
    async () => {
      ...
      try {
        for (let iteration = 0; iteration < ITERATIONS; iteration++) {
          const seed = mixSeed(BASE_SEED, 'mysql-next-type-source', iteration);
