...
import type {PgTableConfig, AutoGeneratedPgColumns, DrizzlePgTableResult} from './types/postgres.types';
 ...
 * ```
 */
export function drizzlePGTable<T, TN extends string = string, TConfig extends PgTableConfig<T> = PgTableConfig<T>>(
    tableName: TN,
    tableConfig?: TConfig,
    type?: ReceiveType<T>
): DrizzlePgTableResult<TN, T, TConfig> {
    ...
    const columns = {} as AutoGeneratedPgColumns<T>;
    for (const prop of typeInfo.properties) {
        // Use provided config if available, otherwise auto-generate
        if (tableConfig && tableConfig[prop.name as keyof T]) {
            (columns as any)[prop.name] = tableConfig[prop.name as keyof T];
        } else {
    ...
    return pgTable(tableName, columns) as unknown as DrizzlePgTableResult<TN, T, TConfig>;
}
