...
import type {MySqlTableConfig, AutoGeneratedMySqlColumns, DrizzleMySqlTableResult} from './types/mysql.types';
 ...
 * ```
 */
export function drizzleMysqlTable<T, TN extends string = string, TConfig extends MySqlTableConfig<T> = MySqlTableConfig<T>>(
    tableName: TN,
    tableConfig?: TConfig,
    type?: ReceiveType<T>
): DrizzleMySqlTableResult<TN, T, TConfig> {
    ...
    const columns = {} as AutoGeneratedMySqlColumns<T>;
    ...
    return mysqlTable(tableName, columns) as DrizzleMySqlTableResult<TN, T, TConfig>;
}
