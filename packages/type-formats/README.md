...
`@mionkit/type-formats` provides runtype functionality such as validation and serialization based on TypeScript types. Unlike libraries like Zod or AJV-formats, which use JavaScript to declare schemas, `@mionkit/type-formats` directly leverages TypeScript types and extracts information at compile time to generate validation and serialization functions.

You can find examples of how to use these formats in the various spec files under the `string` directory.

> **⚡ Fast**:  
> The validation and serialization functions generated by `@mionkit/type-formats` are JIT (Just-In-Time) compiled, ensuring they are highly optimized for performance. This makes them suitable for use in high-throughput applications where efficiency is critical.
...
import {DateTimeString} from '@mionkit/type-formats';
