...
it('validate string alpha', async () => {
    const isType = await isTypeFn<FormatAlpha<{minLength: 3}>>();
    expect(isType('abcdef')).toBe(true);
    expect(isType('ab')).toBe(false);
});
it('get alpha string errors', async () => {
    const typeErrors = await typeErrorsFn<FormatAlpha<{minLength: 3}>>();
...
it('validate string alpha numeric', async () => {
    const isType = await isTypeFn<FormatAlphaNumeric<{minLength: 3}>>();
...
it('get alpha numeric string errors', async () => {
    const typeErrors = await typeErrorsFn<FormatAlphaNumeric<{minLength: 3}>>();
    const format: TypeFormatError = {name: 'stringFormat', formatPath: [], val: ''};
    const expectedError: RunTypeError = {expected: 'string', path: [], format};
...
it('validate string numeric', async () => {
    const isType = await isTypeFn<FormatNumeric<{minLength: 3; maxLength: 5}>>();
...
it('get numeric string errors', async () => {
    const typeErrors = await typeErrorsFn<FormatNumeric<{minLength: 3; maxLength: 5}>>();
    const format: TypeFormatError = {name: 'stringFormat', formatPath: [], val: ''};
    const expectedError: RunTypeError = {expected: 'string', path: [], format};
...
it('validate lowercase string', async () => {
    const isType = await isTypeFn<FormatLowercase<{minLength: 3}>>();
...
it('get lowercase string errors', async () => {
    const typeErrors = await typeErrorsFn<FormatLowercase<{minLength: 3}>>();
...
it('validate uppercase string', async () => {
    const isType = await isTypeFn<FormatUppercase<{minLength: 3}>>();
...
it('get uppercase string errors', async () => {
    const typeErrors = await typeErrorsFn<FormatUppercase<{minLength: 3}>>();
...
it('validate capital string', async () => {
    const isType = await isTypeFn<FormatCapitalize>();
