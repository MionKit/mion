...
function getRouterFunctionName(node, context) {
    ...
    if (node.callee.type !== utils_1.AST_NODE_TYPES.Identifier || !routerFunctions.includes(node.callee.name)) {
        return null;
    }
...
function findFunctionByName(name, context) {
    ...
    const program = sourceCode.ast;
    for (const statement of program.body) {
        if (statement.type === utils_1.AST_NODE_TYPES.FunctionDeclaration && statement.id?.name === name) {
            return statement;
        }
        if (statement.type === utils_1.AST_NODE_TYPES.VariableDeclaration) {
            for (const declarator of statement.declarations) {
                if (declarator.id.type === utils_1.AST_NODE_TYPES.Identifier && declarator.id.name === name) {
...
function getHandlerFunction(node, _functionName, context) {
    ...
    if (handlerArg.type === utils_1.AST_NODE_TYPES.ArrowFunctionExpression || handlerArg.type === utils_1.AST_NODE_TYPES.FunctionExpression) {
        return handlerArg;
    }
    if (handlerArg.type === utils_1.AST_NODE_TYPES.Identifier) {
...
function validateParameterTypes(func) {
    ...
    for (let i = 1; i < func.params.length; i++) {
        ...
        if (param.type === utils_1.AST_NODE_TYPES.Identifier) {
            ...
            if (param.argument.type === utils_1.AST_NODE_TYPES.Identifier && !param.typeAnnotation) {
                missingTypeParams.push(`...${param.argument.name}`);
                missingParamNodes.push(param);
            }
        }
        else if (param.type === utils_1.AST_NODE_TYPES.ArrayPattern || param.type === utils_1.AST_NODE_TYPES.ObjectPattern) {
            if (!param.typeAnnotation) {
...
function getParameterName(param) {
    ...
    else if (param.type === utils_1.AST_NODE_TYPES.RestElement && param.argument.type === utils_1.AST_NODE_TYPES.Identifier) {
...
function getReturnTypeReportNode(func) {
    if ((func.type === utils_1.AST_NODE_TYPES.FunctionDeclaration || func.type === utils_1.AST_NODE_TYPES.FunctionExpression) && func.id) {
        return func.id;
    }
    return func;
}
function getHandlerTypeFromAnnotation(typeAnnotation, context) {
    if (typeAnnotation.typeAnnotation.type === utils_1.AST_NODE_TYPES.TSTypeReference) {
...
function getHandlerTypeFromSatisfies(satisfiesExpression, context) {
    if (satisfiesExpression.typeAnnotation.type === utils_1.AST_NODE_TYPES.TSTypeReference) {
        ...
        return {
            ...
            VariableDeclarator(node) {
                if (node.id.type === utils_1.AST_NODE_TYPES.Identifier) {
                    ...
                    if (!handlerType && node.parent?.type === utils_1.AST_NODE_TYPES.VariableDeclaration) {
            ...
            TSSatisfiesExpression(node) {
                ...
                if (handlerType &&
                    (node.expression.type === utils_1.AST_NODE_TYPES.ArrowFunctionExpression ||
                        node.expression.type === utils_1.AST_NODE_TYPES.FunctionExpression)) {
