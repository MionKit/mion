...
import type {TypeFormatParams, JitFnID, MockOperation, Mutable, JitCompiled, JitFn} from '../types';
import {jitFnHasReturn, jitFnIsExpression, JitFunctions} from '../constants';
import {ReflectionKind} from '@deepkit/type';
import {getFormatterParams, getParamsHash} from './formats';
...
export abstract class JitRunTypeFormatter<P extends TypeFormatParams = any> {
    ...
    getNewPath(paramName?: (string | number) | (string | number)[]): (string | number)[] {
        if (!paramName) return this.parentPath || [];
        if (typeof paramName === 'string' || typeof paramName === 'number')
            return this.parentPath ? [...this.parentPath, paramName] : [paramName];
        const paramArray = paramName as (string | number)[];
        return this.parentPath ? [...this.parentPath, ...paramArray] : paramArray;
    }
    ...
    compile(
        jitFn: JitFn,
        parentCompiler: JitCompiler,
        rt: BaseRunType,
        params?: P,
        parentPath?: (string | number)[],
        vλl?: string
    ): JitCompiled {
        const hash = getParamsHash(undefined, params || this.getParams(rt), this.ignoreJitParams);
        ...
        const jitFnHash = `${JitFunctions.aux.id}_${jitFn.id}_${hash}`;
        ...
        const newJitCompiler: JitCompiler = createJitCompiler(rt, jitFn.id, undefined, jitFnHash, hash) as JitCompiler;
        try {
            const formatterCode = this._compile(jitFn.id, newJitCompiler, rt, params, parentPath, vλl);
            const withReturn = this.handleReturnValues(newJitCompiler, jitFn.id, formatterCode || '');
            newJitCompiler.compile(withReturn);
            parentCompiler.updateDependencies(newJitCompiler as JitCompiled);
        } catch (e) {
    ...
    _compile(fn: JitFnID, comp: JitCompiler, rt: BaseRunType, params?: P, parentPath?: (string | number)[], vλl?: string) {
        const v = comp.vλl;
        comp.vλl = vλl || v;
        this.pushContext(params, parentPath);
        let result: jitCode;
