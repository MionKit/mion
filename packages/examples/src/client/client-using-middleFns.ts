...
const [user, routeError, fatal, middleFnResults, middleFnErrors] = await routes.users.getById('USER-123').call({
  middleFns: {
    auth: middleFns.auth(new HeadersSubset({Authorization: 'myToken-XYZ'})),
  },
});

// routeError is the route's DECLARED errors; each middleware function's declared errors arrive by name
if (routeError?.type === 'user-not-found') console.log('missing:', routeError.errorData?.requestedId);
else if (middleFnErrors?.auth?.type === 'not-authorized') console.log('auth failed:', middleFnErrors.auth.errorData?.reason);
