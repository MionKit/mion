...
export default defineConfig({
  plugins: [
    mionVitePlugin({
      ...
      // 1. batches.emit: write the batches this client build contains (their routes plus their
      //    inline inputFrom mappers) into a manifest the server build consumes. This is the
      //    client<->server batch transport (route validation/serialization metadata itself comes
      //    from the router types at build time, no server needed for that).
