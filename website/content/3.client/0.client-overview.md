...
![Type Safe Animation](/images/type-safe-animation.gif)
...
For routes that require hooks (like authentication), use `callWithHooks()` to pass hook data alongside the route call:
...
For cases like authorization hooks that are required for all requests, we can use the `prefill()` method of `HookSubRequest`. This will automatically persist the subRequest in local storage and prefill any future route calls that require that hook.
...
::note
Handlers registered with `onSuccess` and `onError` are persistent - they will be called after **every** API request that includes this hook, not just once.
::
