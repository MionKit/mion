...
- ✅ Result pattern with `[data, error]` tuple - never throws, always returns.
...
The `call()` method returns a `Result` tuple `[data, error]` - it **never throws**.
...
// calls sayHello route in the server - call() returns [data, error] tuple
const [sayHello, error] = await routes.users.sayHello(john).call();
if (error) {
    console.log('Error:', error.publicMessage);
} else {
...
The `Result` type is a tuple used by `call()` to return either success data or an error without throwing.
...
The result type returned by `callWithHooks()`. Returns a tuple `[data, errors]` containing both route and hooks data/errors.
...
export interface RouteSubRequest<PH extends PublicHandler> extends SubRequest<PH> {
    ...
    /**
     * Calls a remote route and returns a Result tuple.
     * Never throws - errors are always in the result tuple.
     * @returns Promise<[SuccessType, undefined] | [undefined, ErrorType]>
     */
