...
- ✅ Result pattern with `[result, error]` tuple - never throws, always returns.
...
The `call()` method returns a `Result` tuple `[result, error]` - it **never throws**.
...
if (errors.route || errors.hooks.auth) {
    ...
    console.log(results.route); // User object
}
```
...
The result type returned by `callWithHooks()`. Returns a tuple `[results, errors]` containing both route and hooks data/errors.
