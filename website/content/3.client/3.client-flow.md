...
const [result, error, useFnResults, useFnErrors] = await route.call();

// RoutesFlow - arrays of results/errors
const [[result1, result2], [error1, error2], useFnResults, useFnErrors] = await routesFlow([route1, route2]);
```
...
## Using UseFns with RoutesFlow

You can include useFns (like authentication) in your routesFlow:

<code-import path="packages/examples/src/client/workflow-with-useFns.ts" lang="ts" />

::note
UseFns are executed once for the entire routesFlow, not once per route. This is more efficient than making separate calls.
::
