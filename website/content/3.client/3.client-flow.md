---
title: RoutesFlows
description: Execute multiple routes in a single HTTP request with the routesFlow feature.
toc: false
---

A **routesFlow** allows you to execute multiple routes in a single HTTP request. Instead of making separate API calls for each route, you batch them together, reducing network overhead and improving performance.
...
**Coming Soon Feature!**
<br/>
RoutesFlows will support mapping results from one route to the input of another route within the same routesFlow. This will enable GraphQL-like data fetching capabilities — where you can compose complex data queries — while maintaining the simplicity and type safety of mion routesFlows.

```ts
// Future syntax (not yet available)
const [[user, preferences]] = await routesFlow([
...
const [[result1, result2], [error1, error2], linkedFnResults, linkedFnErrors] = await routesFlow([route1, route2]);
```
...
## Using LinkedFns with RoutesFlow

You can include linkedFns (like authentication) in your routesFlow:

<code-import path="packages/examples/src/client/workflow-with-linkedFns.ts" lang="ts" />

::note
LinkedFns are executed once for the entire routesFlow, not once per route. This is more efficient than making separate calls.
::
