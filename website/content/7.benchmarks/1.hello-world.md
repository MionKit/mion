---
toc: false
title: Hello World
description: Performance benchmarks for a simple Hello World endpoint.
---

These benchmarks are based on the [fastify benchmarks](https://github.com/fastify/benchmarks){target="_blank"} repo! in fact our goal is to perform similar to fastify as we considered it the industry standard in terms of performance.

mion is focused on being lightweight and fast so it can be run in serverless environments. We run the benchmarks before every PR gets merged to ensure there is no performance regression.

::tip
**Hello World** benchmark involves mostly routing as there is no validation and is about as fast as each framework can get.
::

## What is tested?

This is a simple hello world scenario, is a good indicator of the router performance and theoretical upper limit of each framework.

<code-import path="packages/examples/src/introduction/myApi.routes.ts" lang="ts" />

::note
**Bun servers are excluded!**   
Bun servers (mion.bun, hono.bun, elysia.bun) are **excluded from the hello world benchmarks** because the benchmarking tool used (autocannon) is not fast enough to accurately measure Bun's performance.
Note that Hono and Elisya have slightly better performance than mion, in bun for this particular case but we can't run test reliable using autocannon.  
::

:spacer

## Benchmarks

* __Machine:__ darwin arm64 | 12 vCPUs | 16.0GB Mem
* __Node:__ `v24.13.0`
* __Run:__ Wed Mar 18 2026 00:18:13 GMT+0000 (Greenwich Mean Time)
* __Method:__ `autocannon -c 100 -d 4 -p 10 localhost:3000` (two rounds; one to warm-up, one to measure)

#### Req (R/s)

:bench-chart{id='hello-requests'}

#### Throughput (Mb/s)

:bench-chart{id='hello-throughput'}

#### Latency (ms)

:bench-chart{id='hello-latency'}

#### Max Memory (Mb)

:bench-chart{id='hello-max-mem'}

#### Memory Series (MB)

:bench-chart{id='hello-mem-series'}

## Results Table

|           | Version   | Router | Req (R/s)    | Latency (ms) | Output (Mb/s) | Max Memory (Mb) | Max Cpu (%) | Validation | Description                                               |
| :--       | --:       | --:    | :-:          | --:          | --:           | --:             | --:         | :-:        | :--                                                       |
| fastify   | 5.7.4     | ✓      | 118480.0     | 7.94         | 21.24         | 121             | 108         | ✓          | Fastify with native JSON Schema validation                |
| http-node | 16.18.0   | ✗      | 118480.0     | 7.94         | 21.13         | 149             | 108         | ✓          | bare node http server with Zod validation                 |
| **mion**  | **0.6.2** | **✓**  | **102352.0** | **9.27**     | **19.91**     | **155**         | **109**     | **✓**      | **Automatic validation and serialization out of the box** |
| hono      | 3.12.6    | ✓      | 97696.0      | 9.73         | 16.02         | 153             | 109         | ✓          | hono node server with Zod validation                      |
| hapi      | 21.4.4    | ✓      | 83184.0      | 11.50        | 14.84         | 234             | 111         | ✓          | Hapi with Zod validation                                  |
| express   | 5.2.1     | ✓      | 72064.0      | 13.35        | 12.85         | 128             | 109         | ✓          | Express with Zod validation                               |

:button-link[Benchmarks Repo]{icon="grommet-icons:github" href="https://github.com/MionKit/Benchmarks" target="_blank"}

<!-- code-import-timestamp 1772057398350 -->
