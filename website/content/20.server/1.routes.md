...
- **`query()`** — for read-only operations. The client automatically sends small payloads via `GET` with `?data=base64url` instead of `POST` with a body. This enables browser and CDN caching, and avoids CORS preflight requests.
- **`mutation()`** — for operations that modify data. The client always sends these as `POST` with a body.
- **`route()`** — no explicit intent, client uses POST (data is sent in the body). Does not participate in cache semantics.

<code-import path="packages/examples/src/router/query-mutation-definition.routes.ts" lang="ts" />

::tip
Declaring intent with `query()` and `mutation()` is recommended for all new APIs. It makes your code more readable and enables future optimizations like caching layers.
::

### Full Example with Query and Mutation

<code-import path="packages/examples/src/router/query-mutation-full.routes.ts" lang="ts" />
