...
**Example:**
[`@mionkit/node`](/platforms/node-js) uses node's native `IncomingMessage` and `ServerResponse` while [`@mionkit/aws`](/platforms/aws-lambda) uses `APIGatewayEvent` and `APIGatewayProxyResult`. But routes and middleFns will always receive a standard MionRequest & MionResponse.
