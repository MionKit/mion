...
Type formats from `@mionkit/type-formats` provide additional semantic information that enables more precise column mapping. For example, `FormatInteger` maps to an integer column rather than a floating-point column, and `FormatEmail` uses the appropriate varchar length for email addresses.
