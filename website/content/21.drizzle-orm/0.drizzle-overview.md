...
::note
Column types are derived from Typescript types and [`@mionkit/type-formats`](../run-types/type-formats) - when you use format types like `FormatUUIDv7`, `FormatEmail`, `FormatInteger`, etc., the package automatically selects the most appropriate database column type for each database.
::
...
|----------|----------|--------|
| PostgreSQL | `toDrizzlePGTable<T>(name, config?)` | `@mionkit/drizzle` |
| MySQL | `toDrizzleMySqlTable<T>(name, config?)` | `@mionkit/drizzle` |
| SQLite | `toDrizzleSqliteTable<T>(name, config?)` | `@mionkit/drizzle` |
...
import type {FormatUUIDv7, FormatEmail} from '@mionkit/type-formats';
```
