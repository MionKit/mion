...
::note
The key feature is that column types are derived from [`@mionkit/type-formats`](../run-types/type-formats) - when you use format types like `StrUUIDv7`, `StrEmail`, `NumInteger`, etc., the package automatically selects the most appropriate database column type for each database.
::
