...
::note
Column types are derived from Typescript types and [`@mionkit/type-formats`](../run-types/type-formats) - when you use format types like `StrUUIDv7`, `StrEmail`, `NumInteger`, etc., the package automatically selects the most appropriate database column type for each database.
::
