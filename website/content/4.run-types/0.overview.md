...
description: Compilation library that generates optimized validation, serialization, and mocking functions from TypeScript types.
toc: false
---

mion's RunTypes engine (`@ts-runtypes/core`) is a powerful compilation library that generates optimized validation, serialization, and mocking functions directly from TypeScript types. Unlike schema-based libraries like Zod or AJV, run-types leverages TypeScript's type system at compile time to generate highly efficient runtime code.
...
- ✅ **Precompiled Functions**: Validation and serialization functions are generated at build time by the mion Vite plugin — no runtime code generation
...
Returns a value → JSON-string encoder. It walks the type and makes every member JSON-safe (Date → ISO string, BigInt → string, Map/Set → entries) before stringifying, so it handles the types that plain `JSON.stringify` drops or mangles.

<code-import path="packages/examples/src/run-types/json-stringify.ts" lang="ts" />

::note
`createJsonEncoder` traverses the object in generated code rather than calling `JSON.stringify` directly, and it does **not** mutate the input.
::

#### `createJsonDecoder<T>()`

Returns a JSON-string → value decoder — the inverse of `createJsonEncoder`. It parses the string and revives every member back to its declared type (ISO string → Date, entries → Map).
...
- **Pure functions** embedded into the generated validation/serialization code that should not include compiler artifacts
...
Pure functions are methods that do not have side effects, don't depend on variables outside their scope or module imports, and can be embedded into the generated validation/serialization code.

They are a `@ts-runtypes` feature — mion has no pure-function registry of its own. Register them with `registerPureFn(id, fn)` from `@ts-runtypes/core`. The id is a `<namespace>::<name>` string literal, and the function must be an inline literal: the build scanner extracts the body and AOT-compiles it, so it needs to see both at the call site. The **namespace** groups related functions so different libraries can register their own without naming conflicts:
