...
Register them with `registerMionPureFn(namespace, factory)` from `@mionjs/core`. The factory returns the actual function, so any constants it needs are created once at registration time. The **namespace** groups related functions, so different libraries or modules can register their own pure functions without naming conflicts:
