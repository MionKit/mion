...
RunTypes relies on TypeScript's type metadata being available at runtime. There are common pitfalls that can prevent this from working correctly. mion provides [ESLint rules](/introduction/eslint-rules) to catch these issues at development time.
...
::note
Use the [`@mionkit/no-type-imports`](/introduction/eslint-rules#mionkitno-type-imports) ESLint rule to catch this issue.    
This Rule will only check Types that are used in routes/linkedFns.
::
...
RunTypes needs explicit type annotations on route/linkedFn parameters and return types. Without them, TypeScript cannot emit the type metadata needed for runtime validation.
...
::note
Use the [`@mionkit/strong-typed-routes`](/introduction/eslint-rules#mionkitstrong-typed-routes) ESLint rule to catch this issue.
::
...
::note
Use the [`@mionkit/no-typeof-runtype`](/introduction/eslint-rules#mionkitno-typeof-runtype) ESLint rule to catch this issue.
::
...
::note
Use the [`@mionkit/no-unreachable-union-types`](/introduction/eslint-rules#mionkitno-unreachable-union-types) ESLint rule to catch this issue.   
This rule will only check Types that are used in routes/linkedFns.
::
