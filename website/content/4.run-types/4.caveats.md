...
This Rule will only check Types that are used in routes/hooks.
::
...
RunTypes needs explicit type annotations on route/hook parameters and return types. Without them, TypeScript cannot emit the type metadata needed for runtime validation.
