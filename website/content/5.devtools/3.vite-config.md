...
mionPlugin({
    ...
    server?: {
        ...
        viteConfig?: string;            // Server's vite.config.ts for vite-node — required when
                                        // your start script imports virtual:mion-aot/* so the
                                        // pre-pass child can resolve those modules. Auto-discovered
                                        // when the start script lives next to a vite.config.ts.
...
::note
The `buildStart` AOT pre-pass runs in `vite dev` and `vite build` whenever any source file imports `virtual:mion-aot/*`. Setting `aotCaches: true` and providing `server.startScript` is enough to opt in — no separate "build vs dev" toggle is needed.
::

::note
**Test envs opt out automatically.** When `process.env.VITEST` is set or `NODE_ENV === 'test'`, the pre-pass is skipped and `virtual:mion-aot/caches` falls back to a globalThis-backed shim that lets tests mock the cache directly.
::
