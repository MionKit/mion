...
mionPlugin({
    ...
    server?: {
        ...
        port?: number;                  // Port to poll for readiness (childProcess mode)
...
When both client and server are managed by Vite. The client config starts the server process and generates AOT caches. In `childProcess` mode the server stays running for dev; in `middleware` mode it runs inside the same Vite process (e.g., Nuxt).
...
::code-group
<code-import path="packages/examples/src/codegen/vite-client.config.ts" lang="ts [Client (buildOnly)]" />
