...
- Types not exposed in route/useFn parameters or return types will not be compiled
...
- **Fail-fast validation**: If any route/useFn is missing from the AOT cache, an `AOTCacheError` is thrown immediately
...
::warning
When `aot: true` is set, ALL routes and useFns MUST be present in the AOT cache. If any are missing, the router will throw an `AOTCacheError` instead of falling back to JIT compilation.
::
