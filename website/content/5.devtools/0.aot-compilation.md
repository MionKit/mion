...
1. **`buildStart`** — The plugin spawns a `vite-node` child process that runs your server start script (or uses `ssrLoadModule` in `viteSSR` mode)
...
The `server.mode` determines how the server process is managed:
- **`onlyAOT`**: Spawn child, generate caches, kill child (best for production builds)
- **`IPC`**: Spawn child, generate caches, keep server running (best for development/testing)
- **`viteSSR`**: Load in same Vite process (for frameworks like Nuxt)
