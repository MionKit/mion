...
Client router are automatically added when calling `initMionRouter`.

It is also required to export the `RemoteApi` type returned from `initMionRouter` so it can be used by the client.
